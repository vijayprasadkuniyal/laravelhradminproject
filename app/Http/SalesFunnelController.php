...
//         }
  ...
  $renewal_amount  =  $total_upcoming_renewal
   ...
   ->sum('package_amount');
    ...
    // Calculate accuracy
    if ($call_connected > 0) {
        $accuracy = round(($mature_followups->count() / $call_connected) * 100);
...
public function lead_sale(Request $request)
{
    ...
    $query = $data->whereIn('package_info.exe_id', $emp_managers)
        ->select(
            ...
            'leads_info.category_id',
            DB::raw('COUNT(DISTINCT leads_info.id) as lead_count')
        )
        ->groupBy('package_info.group_id', 'leads_info.category_id')
        ->get();
...
public function lead_sale_price(Request $request)
{
    ...
    $query = $data->whereIn('package_info.exe_id', $emp_managers)
        ...
        ->get()
        ->map(function($item) {
            $item->average_value = $item->unique_package_count > 0 ? $item->total_sent_value / $item->unique_package_count : 0;
            return $item;
        });
