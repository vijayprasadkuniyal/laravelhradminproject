      ...
      public function __construct()
      {
       ...
       public function dm_dashboard_count(Request $request)
       {
        $count = 0;
           ...
           $organic_enquiry = $this->salesdb->table('enquiry_info')->where('source_type','Website');
           ...
           $lead_quality = DB::connection('sales_db')->table('enquiry_info')->whereIn('followup_status',[18,19,20,21,6,10]);

            $total_inactive_package = $this->salesdb->table('package_info')
                ->where('package_status',2)
                ->whereColumn('package_end_date', '>', 'package_end_expected_date');
            
             if ($request->group) {
               $lead_quality->where('group_id', $request->input('group'));
           ...
           if ($request->product) {
               $lead_quality->where('product_id', $request->input('product'));
           ...
           if ($request->service) {
               $lead_quality->where('service_id', $request->input('service'));
            ...
            if ($request->category) {
               $lead_quality->where('category_id', $request->input('category'));
           ...
           if ($request->start_date && $request->end_date) {
               ...
               $lead_quality->whereDate('created_date', '>=', $startDate)
               ...
               $total_enquiry->whereDate('created_date', '>=', $startDate)
                ...
                ->whereDate('created_date', '<=', $endDate);
                $package_over_running->whereDate('package_end_expected_date', '>=',$startDate)
                ->whereDate('package_end_expected_date', '<=', $endDate);
               ...
               $package_over_running->whereMonth('package_end_expected_date',Carbon::now()->month);
               ...
               $lead_quality->whereDate('created_date', $currentDate);
               
              }
       
           
           $total_enquiry_count = $total_enquiry->count();
           $total_valid_enquiry = $total_enquiry->whereNotIn('followup_status',[3,5,17,4,7,14,8,16])->count();
           if($total_enquiry_count>0){
               $total_valid_lead_percent =  $total_valid_enquiry/$total_enquiry_count*100;
               
           }
           else{
               $total_valid_lead_percent = 0;
           ...
           $total_social_media_enquiry = $social_media_enquiry
            ->whereNotIn('source_type',['TF','Organic','Adword','Website','App','CS'])->count();

           foreach($package_over_running_data as $row){
            if($row->package_end_expected_date !='' && $row->package_end_date==''){
        ...
        if (!$check_package_renew_or_not) {
           ...
           'social_media_enquiry'=>$total_social_media_enquiry,'renewal_gone'=>$no_of_clients,'valid_lead_percent'=>round($total_valid_lead_percent),'lead_quality_count'=>$lead_quality->count());
       ...
       public function dm_dashboard_enquiry_inner_page(Request $request)
{
    try {
        ...
        if ($request->type) {
            switch ($request->type) {
                case 'not-sent':
                    if (!$request->enq_status) $query->where('enquiry_info.enq_status', 3);
                    break;
                case 'organic-leads':
                    if (!$request->source_type) $query->where('enquiry_info.source_type', 'Website');
                    break;
                case 'total-sent':
                    if (!$request->enq_status) $query->where('enquiry_info.enq_status', 1);
                    break;
                case 'call-generated':
                    if (!$request->source_type) $query->where('enquiry_info.source_type', 'TF');
                    break;
                case 'leas-sale':
                    if (!$request->enq_status) $query->where('enquiry_info.enq_status', 1);
                    break;
                case 'social_media_enquiry':
                    if (!$request->source_type) $query->whereNotIn('source_type', ['TF', 'Organic', 'Adword', 'Website', 'App', 'CS']);
                    break;
            }
        }
        ...
        // Add the orderBy clause for pagination
        $query->orderBy('enquiry_info.created_date', 'desc'); // Change to your preferred column and order direction
        ...
        if ($request->download) {
            ...
            $response = new StreamedResponse(function() use ($query) {
                ...
                $query->chunk(1000, function($results) use ($handle) {
        ...
        // Paginate results for normal response
        ...
        \Log::error('Error in dm_dashboard_enquiry_inner_page: ' . $e->getMessage());
      ...
      public function package_over_running(Request $request) {
        ...
        // Start query builder
        $data = $this->salesdb->table('package_info')->where('package_status',1)->where('package_end_expected_date','!=','')->where('package_end_date','=','')->whereDate('package_end_expected_date', '<', Carbon::now()->format('Y-m-d'));
        ...
        if ($request->start_date && $request->end_date) {
            $data->whereDate('package_end_expected_date', '>=', $request->start_date)
                 ->whereDate('package_end_expected_date', '<=', $request->end_date);
        } else {
            //$currentDate = Carbon::now()->format('Y-m-d');
            $data->whereMonth('package_end_expected_date', Carbon::now()->month);
        }
       ...
       if($request->download){
        ...
        foreach ($data_array as $row) {
            if ($row->package_end_expected_date && Carbon::now()->greaterThan(Carbon::parse($row->package_end_expected_date))) {
                ...
                $client = $this->salesdb->table('company_info')->where('comp_id', $row->comp_id)->first();
...
public function lead_feedback(Request $request){
    try {
        $lead_feedback = DB::connection('sales_db')->table('client_feedback')
            ->leftjoin('enquiry_info', 'client_feedback.enq_id', '=', 'enquiry_info.id')
            ->leftJoin('group_names', 'enquiry_info.group_id', '=', 'group_names.group_id')
            ->leftJoin('product', 'enquiry_info.product_id', '=', 'product.id')
            ->leftJoin('product_service', 'enquiry_info.service_id', '=', 'product_service.id')
            ->leftJoin('product_category', 'enquiry_info.category_id', '=', 'product_category.id')
...
public function get_sent_percent_calculation_lmart($dept_id,$dashboard_type){
    ...
    if ($dept_id == 10 || $dept_id == 9 || $dept_id == 4 ) {
        if($dashboard_type == 'dm_dashboard'){
            $total_enquiry = DB::connection('sales_db')->table('enquiry_info')
            ->where('product_id', 1)
            ->whereDate('created_date', Carbon::now()->format('Y-m-d'))
            ->count();

        $total_sent = DB::connection('sales_db')->table('enquiry_info')
            ->where('enq_status', 1)
            ->where('product_id', 1)
            ->whereDate('created_date', Carbon::now()->format('Y-m-d'))
            ->count();

        $targetSentPercent = DB::table('save_company_target')
            ->where('department_id', 10)
            ->where('attribute_id', 3)
            ->where('subattribute_id',5)
        ...
        if ($total_enquiry > 0) {
            $percent = round(($total_sent / $total_enquiry) * 100);
            
            if ($targetSentPercent) {
                if ($percent < $targetSentPercent->child_attribute_value) {
                    return response()->json([
                        'status' => 200,
                        'message' => 'Lead Sent Percent Of Lmart Is Less Than ' . $targetSentPercent->child_attribute_value
                    ]);
                }
            } else {
                return response()->json([
                    'status' => 201,
                    'message' => 'Target percentage not found for the specified financial year.'
                ]);
            }
            
            return response()->json([
                'status' => 201,
                'message' => 'Sent Percent: ' . $percent . '%'
            ]);
        } else {
            return response()->json([
                'status' => 201,
                'message' => 'No enquiries found for today.'
            ]);
        }
    }
...
public function dm_department_employees(){
    $data = DB::table('emp_basic_info')->where('dept_id',10)->where('emp_status',1)->get(['emp_id','emp_fname','emp_lame']);
    return response()->json(['status'=>200,'data'=>$data]);
}
public function lead_not_sent_due_to_quality(Request $request){
    $data_array = [];
    $data = DB::connection('sales_db')->table('enquiry_info')->whereIn('followup_status',[3,5,17,4,7,14,8,16]);
    if($request->group){
        $data->where('group_id',$request->group);
    ...
    foreach($query as $row){
            ...
            $data_array[] = array('id'=>$row->id,'product'=>$product->product_name??'','group'=>$group->name??'','category'=>$category->category_name??'','service'=>$service->service_name??"",'customer_name'=>$customer->name??"",'created_date'=>$row->created_date,'status'=>$enq_status->name??'');
             }
             return  response()->json(['status'=>200,'data'=>$data_array,'last_page'=>$query->lastPage()]);
        
    }
    
 public function get_renewal_gone_due_to_lead_data(Request $request)
{
    $array_data = [];
    
    $packagesQuery = $this->salesdb->table('package_info')
        ->where('package_status', 2)
        ->whereColumn('package_end_date', '>', 'package_end_expected_date');


    if ($request->group) {
        $packagesQuery->whereRaw('FIND_IN_SET(?, group_id)', [$request->input('group')]);
    }

    if ($request->start_date && $request->end_date) {
        $startDate = $request->input('start_date');
        $endDate = $request->input('end_date');
        $packagesQuery->whereDate('package_end_expected_date', '>=', $startDate)
                      ->whereDate('package_end_expected_date', '<=', $endDate);
    }

   
    $clientIds = $packagesQuery->select('client_id')->distinct()->pluck('client_id');
    ...
    foreach ($clientIds as $client_id) {
        ...
        if ($get_last_package) {
            ...
            if (!$check_package_renew_or_not) {
                $clientIdArray[] = $client_id;
            }
        }
    }
    
    foreach ($clientIdArray as $client_id) {
        $client_data = DB::connection('sales_db')->table('client_info')->where('client_id', $client_id)->first();
        
        if ($client_data) {
            $group_name = '';
            if ($client_data->group_id) {
                $group_names = DB::connection('sales_db')->table('group_names')->where('group_id', $client_data->group_id)->first();
                if ($group_names) {
    ...
    return response()->json(['status' => 200, 'data' => $array_data]);
}
