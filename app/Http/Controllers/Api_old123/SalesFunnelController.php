...
//use App\Exports\ManagerCollectionExport;
...
public function count_sales_data(Request $request) {
    ...
    $data = DB::connection('sales_db')->table('clients_followup_log')
        ->leftjoin('company_info', 'company_info.comp_id', 'clients_followup_log.comp_id')
        ->leftjoin('guest_user_details', 'guest_user_details.id', 'clients_followup_log.client_id')
        ->select('clients_followup_log.*', 'company_info.client_name', 
            'company_info.group_id as company_info_group', 
            'guest_user_details.group_id as guest_group')
        ->whereIn('clients_followup_log.created_by', $emp_managers)
        ->whereIn('clients_followup_log.id', function ($query) {
            $query->select(DB::raw('MAX(id)'))
                ->from('clients_followup_log')
                ->groupBy('clients_followup_log.client_id','clients_followup_log.comp_id');
        });
    ...
    $mature_followups = DB::connection('sales_db')
    ...
    $upcoming_renewal = DB::connection('sales_db')->table('package_info')
    ...
    // Apply filters
    if ($request->group) {
     $upcoming_renewal->whereRaw('FIND_IN_SET(?, group_id)', [$request->group]);
    ...
    $mature_followups->where('cfl.city_id', $request->group);
   ...
   // $total_upcoming_renewal->where('product_id',$request->product);
    ...
    if ($request->product) {
        $upcoming_renewal->where('product_id', $request->product);
        ...
        $mature_followups->where('cfl.product_id', $request->product);
    }
    if ($request->service) {
        $upcoming_renewal->whereRaw('FIND_IN_SET(?, service_id)', [$request->service]);
        ...
        $mature_followups->where('cfl.service_id', $request->service);
        
    }
    if ($request->category) {
         $upcoming_renewal->whereRaw('FIND_IN_SET(?, category_id)', [$request->category]);
       ...
       // $total_upcoming_renewal->whereRaw('FIND_IN_SET(?, category_id)', [$request->category]);
        ...
        $mature_followups->where('cfl.category_id', $request->category);
    }
    if ($request->start_date && $request->end_date) {
         $upcoming_renewal->whereBetween('created_date', [$request->start_date, $request->end_date]);
        ...
        $carray_forward->whereDate('clients_followup_log.created_date', '>=', $request->start_date)
         ...
         $due_amount->whereDate('due_date', '>=', $request->start_date)
        ...
        $mature_followups->whereDate('cfl.next_followup_date', '>=',$request->start_date)
        ->whereDate('cfl.next_followup_date', '<=', $request->end_date);
        
    } else {
        $mature_followups->whereDate('cfl.next_followup_date', Carbon::now()->format('Y-m-d'));
       ...
       // $total_upcoming_renewal->whereMonth('package_end_expected_date', Carbon::now()->month);
        ...
        $due_amount->whereBetween('due_date', [
            Carbon::now()->subDays(40)->toDateString(),  
           Carbon::now()->toDateString()           
         ]);
    ...
    //$mature_followup = $mature_followups->count();
    $dead_followup = $total_call_connected->clone()->where('followup_id', 15)->count();
...
//         }
   ...
   $renewal_data  =  $upcoming_renewal
   ->whereIN('exe_id',$emp_managers)
   ->count();

  $renewal_amount  =  $upcoming_renewal
   ->whereIN('exe_id',$emp_managers)
   ->sum('package_amount');


    // Calculate accuracy
    $total_followups = $followup_for_meeting + $followup_for_payment + $business_proposal;

      if ($total_followups > 0) {
           $accuracy = round(($mature_followups->count() / $total_followups) * 100);
...
public function sales_dashboard_data(Request $request) {
    ...
    if ($request->manager && !$request->employee) {
        ...
        $emp_managers = array_unique($emp_managers);
            ...
            $not_sent_groups = DB::connection('sales_db')->table('enquiry_info')
            ->where('sent_count', '<', 4)
        ...
        $total_lead =  DB::connection('sales_db')->table('service_info')
                        ->join('package_info','service_info.package_id','package_info.package_id')
                            ->select('package_info.package_id','package_info.group_id','package_info.product_id','service_info.package_id',
                            'service_info.category_id','service_info.service_id','service_info.total_lead','service_info.created_date','service_info.sent_lead','package_info.exe_id');

          $lead_sale_price = DB::connection('sales_db')
                ->table('service_info')
                ->join('package_info', 'package_info.package_id', '=', 'service_info.package_id')
                ->selectRaw('
           ...
           ->where('wallet_info.balance_amount','>',0)
      ...
      if($request->product){
        $total_lead->where('package_info.product_id',$request->product);
        ...
        //$total_upcoming_renewal->where('product_id',$request->product);
        $not_sent_groups->where('enquiry_info.product_id', $request->product);
      ...
      if($request->group){
         $buffer_amount->where('client_info.group_id',$request->group);
         $records->whereRaw('FIND_IN_SET(?, group_id)', [$request->group]);
         $total_lead->whereRaw('FIND_IN_SET(?, package_info.group_id)', [$request->group]);
         $due_amount->whereRaw('FIND_IN_SET(?, group_id)', [$request->group]);
         $client_not_found->where('group_id',$request->group);
         $lead_sale_price->whereRaw('FIND_IN_SET(?, group_id)', [$request->group]);
        ...
        $not_sent_groups->where('enquiry_info.group_id', $request->group);
      ...
      if($request->service){
         $records->where('service_id',$request->service);
         $total_lead->where('service_info.service_id',$request->service);
       ...
       // $total_upcoming_renewal->whereRaw('FIND_IN_SET(?, service_id)', [$request->service]);
        ...
        $due_amount->whereRaw('FIND_IN_SET(?, service_id)', [$request->service]);
        $not_sent_groups->where('enquiry_info.service_id', $request->service);
        $extra_sent->where('package_info.service_id', $request->service);

        
      }
      if($request->category){
         $records->whereRaw('FIND_IN_SET(?, category_id)', [$request->category]);
         $total_lead->where('service_info.category_id',$request->category);
        ...
        $due_amount->whereRaw('FIND_IN_SET(?, category_id)', [$request->category]);
        $not_sent_groups->where('enquiry_info.category_id', $request->category);
        $extra_sent->where('service_info.category_id', $request->category);

      }

      if($request->start_date && $request->end_date){
         $lead_sale_price->whereDate('package_info.created_date', '>=',$request->start_date)
         ->whereDate('package_info.created_date', '<=', $request->end_date);

         $due_amount->whereDate('due_date', '>=', $request->start_date)
             ->whereDate('due_date', '<=', $request->end_date);

        $total_lead->whereDate('service_info.created_date', '>=', $request->start_date)
        ...
        ->whereDate('created_date', '<=', $request->end_date);
         ...
         $today_followup->whereDate('clients_followup_log.next_followup_date', '>=',$request->start_date)
        ->whereDate('clients_followup_log.next_followup_date', '<=', $request->end_date);
        ...
        $records->whereDate('created_date', '>=',$request->start_date)
                ->whereDate('created_date', '<=', $request->end_date);
        //$lead_sale->whereDate('leads_info.sent_date', '>=', $request->start_date)
             //->whereDate('leads_info.sent_date', '<=', $request->end_date);

        $not_sent_groups->whereDate('enquiry_info.created_date', '>=', $request->start_date)
        ...
        $total_business_lead->whereDate('created_at', '>=', $request->start_date)
         ...
         $lead_sale_price->whereMonth('package_info.created_date', Carbon::now()->month)
         ->whereYear('package_info.created_date', Carbon::now()->year);

          $total_lead->whereMonth('service_info.created_date', Carbon::now()->month)
          ->whereYear('service_info.created_date', Carbon::now()->year);
        ...
        $not_sent_groups->whereMonth('enquiry_info.created_date', Carbon::now()->month)
            ->whereYear('enquiry_info.created_date', Carbon::now()->year);

        $extra_sent->whereMonth('package_info.created_date', Carbon::now()->month)
            ->whereYear('package_info.created_date', Carbon::now()->year);
         ...
         $total_lead_sum = $total_lead->whereIn('package_info.exe_id',$emp_managers)->sum('service_info.total_lead');
        $total_sent_lead = $total_lead->whereIn('package_info.exe_id',$emp_managers)->sum('service_info.sent_lead');
       if($request->manager || $request->employee){
        ...
        $active_clients_counts = $active_clients->count();
        $active_packages_counts = $active_packages->count();
      ...
      if($followup_count>0){
        ...
        $renewal_data  =  $records
                           ->whereIN('exe_id',$emp_managers)
                           ->count();

        $renewal_amount  =  $records
                           ->whereIN('exe_id',$emp_managers)
                           ->sum('package_amount');
    ...
    ->count();
       ...
       $regular_client_dead = DB::connection('sales_db')
          ...
          $buffer_amount_sum = $buffer_amount->sum('balance_amount');
          ...
          $missing_followup_sum =    DB::connection('sales_db')
                                    ...
                                    $groupedData = $not_sent_groups->select(
                                    ...
                                    $extra_sent_data = $extra_sent->whereIn('package_info.exe_id', $emp_managers)->get();
       ...
       foreach ($categorydata as $cat) {
        ...
        $total_lead_sale_query = $lead_sale_price->whereIn('package_info.exe_id',$emp_managers)->get();
        $totalSalePrice = 0;
        $totalLeads = 0;
        $totalPrice = 0;
        $packageData = [];
        ...
        foreach ($total_lead_sale_query as $row) {
        ...
        foreach ($packageData as $packageId => $data) {
            $totalPrice += $data['per_lead_price'];
    
            if ($data['total_lead'] > 0) {
                $totalSalePrice += $data['per_lead_price'];
            }
        ...
        //$total_lead_count_sum = $total_lead->get();
        

        //if (count($total_lead_count_sum)>0) {
            //return ''
        ...
        $data_list = array('renew' => $renewal_data,'renewal_amount'=>$renewal_amount,
                    ...
                    'today_collection'=>$today_collection,'followup_sum'=>$today_followup_sum,'total_active_clients'=>$active_clients_counts,
                    'total_active_packages'=>$active_packages_counts,'group_loss'=>$totalLoss,
                    'category_loss'=>$total_diff_sent_count_in_cat,'lead_sale_price'=>round($totalSalePrice),
                    'client_not_found'=>$client_not_found->count(),'missing_followup'=>$missing_followup,'business_lead_count'=>$total_business_lead_count,
                    'buffer_amount'=>$buffer_amount_sum,'total_lead'=>$total_lead_sum,'lead_sale'=>$total_sent_lead);
...
public function get_total_collection(Request $request) {
    ...
    $query->whereYear('payment_history.created_date', $request->year ?? Carbon::now()->year);
    $query->whereIn('payment_history.exe_id', $emp_managers);

    // Fetch Data
    $data = $query
        ->select(
            DB::raw('YEAR(payment_history.created_date) as year'),
            DB::raw('MONTH(payment_history.created_date) as month'),
            DB::raw('SUM(payment_history.paid_amount) as total_collection')
        )
        ->groupBy('year', 'month')
        ->get();
    ...
    $result = collect(range(1, 12))->map(function ($month) use ($data, $months, $year) {
        $monthData = $data->firstWhere('month', $month);
        return [
            'year' => $year,
            'month' => $months[$month],
            'total_collection' => $monthData ? $monthData->total_collection : 0,
        ];
    });
...
public function get_regular_client_details(Request $request){
   ...
   // return $dates;
    ...
    public function show_business_lead_data($emp_id,Request $request) {
        ...
        if($request->category){
             $get_data->where('category',$request->category);
...
public function get_followups_details(Request $request) {
    //return DB::connection('sales_db')->table('clients_followup_log')->where('client_id',120)->get();
    $data_array = [];
    ...
    if($request->type =='meeting_followup' || $request->type =='payment_followup' || $request->type =='business_proposal'){
        $data->whereIn('clients_followup_log.id', function ($query) {
            $query->select(DB::raw('MAX(id)'))
                ->from('clients_followup_log')
                ->groupBy('clients_followup_log.client_id','clients_followup_log.comp_id');
        });
    ...
    if ($request->start_date && $request->end_date) {
        ...
        if($request->type == 'no_of_call_connected' || $request->type == 'dead_followup'){
    ...
    if($request->download=='csv'){
        ...
        return Excel::download(new class($data_array) implements FromArray {
            ...
            public function array(): array
            {
                ...
                return array_merge(
                    [['Client ID', 'Client Name', 'Company Name', 'Group', 'Product', 'Category', 'Service', 'Follow-up Status', 'Created Date',  'Employee Name', 'Manager', 'No of Package Buys', 'Package Buy Last Date', 'Package Amount',  'Next Follow-up Date',]], // Headers
                    $this->data // Data rows
                );
            }
        }, 'follow_up_data.csv');
...
public function carry_forward_followups(Request $request) {
    ...
    ->select(
        ...
        'guest_user_details.group_id as guest_group_id', 
        'company_info.group_id as company_group_id',
        'company_info.mobile_no as company_mobile',
        'guest_user_details.mobile_no as guest_business_mobile',
        ...
        $no_of_times_carry_forward = max(0, $no_of_times_carry_forward-1);
            ...
            if ($row->client_type == 1) {
                ...
                $group = DB::connection('sales_db')->table('group_names')
                    ...
                    ->first();
                $group_name = $group ? $group->name : '';
                $mobile_no = $row->guest_business_mobile;
            } else {
                $client_name = 'Unknown';
                $group_name = '';
                $company_name = '';
...
public function get_active_clients_details(Request $request) {
    ...
    if($request->manager || $request->employee){
        ...
        $query = $data->orderBy('comp_id','DESC')->paginate(10);
...
public function get_active_packages_list(Request $request) {
    ...
    if($request->manager || $request->employee){
        ...
        $query = $data->orderBy('package_info.package_id','DESC')->paginate(10);
...
public function group_loss(Request $request)
{
    ...
    if ($request->service) {
        $data->where('enquiry_info.service_id', $request->service);
        $extra_sent->where('package_info.service_id', $request->service);
    }

    if($request->category){
        $data->where('enquiry_info.category_id', $request->category);
        $extra_sent->where('service_info.category_id', $request->category);
    ...
    // Pagination
    $groupLoss = collect($groupLoss)->values(); // Convert array to a collection
    $perPage = 10;
    $currentPage = $request->get('page', 1);
    $paginatedData = new \Illuminate\Pagination\LengthAwarePaginator(
        $groupLoss->slice(($currentPage - 1) * $perPage, $perPage)->values(),
        $groupLoss->count(),
        $perPage,
        $currentPage,
          ...
          foreach ($teamMembersMapping as $managerId => $members) {
              ...
              if ($request->start_date && $request->end_date) {
                //return 'lll';
                  ...
                  $total_assigned_target = $call_connected_count;
...
public function lead_sale(Request $request)
{
    ...
    if ($request->category) {
     ...
     if ($request->start_date && $request->end_date) {
        ...
        $data->whereMonth('created_date', Carbon::now()->month)->whereYear('created_date', Carbon::now()->year);
    }
     $total_data =  $data->whereIn('exe_id', $emp_managers)->orderBy('package_id','DESC')->paginate(10);
...
public function lead_sale_price(Request $request)
{
    ...
    foreach ($data_array as $row) {
        ...
        // Use a set to ensure that you don't double-count packages if they belong to multiple groups
        $processed_groups = [];
        
        foreach ($group_ids as $group_id) {
            if ($request->group && $group_id != $request->group) {
                // Skip groups that do not match the requested group
                continue;
            }
            
            $groupCategoryKey = $group_id . '-' . $row->category_id;
            
            // Skip already processed group/category pairs to prevent duplicate counting
            if (in_array($groupCategoryKey, $processed_groups)) {
                continue;
            }
            
            $processed_groups[] = $groupCategoryKey;

            // Initialize the group/category data if not already set
            ...
            // Update total sums
            $total_lead_sum += $lead_contribution;
            $total_num_of_packages += 1;
        }
    }

    // Calculate lead sale price for each group/category
    ...
    // Calculate the overall lead sale price
    $overall_lead_sale_price = $total_num_of_packages > 0 ? $total_lead_sum / $total_num_of_packages : 0;

    // Prepare final result with group names and category names
    $final_result = [];
    ...
    foreach ($result as $key => $value) {
        ...
        $final_result[] = [
            ...
            'np_of_packages' => $value['num_of_packages'],
            'lead_sum' => $value['total_lead_sum'],
        ];
    }
    ...
    return response()->json([
        ...
        'overall_lead_sale_price' => round($overall_lead_sale_price), // Add overall lead sale price
    ]);
}
...
public function due_amount_details(Request $request){
    ...
    if ($request->start_date && $request->end_date) {
        $data->whereDate('due_date', '>=', $request->start_date)
             ->whereDate('due_date', '<=', $request->end_date);
    }
    else{
    ...
    $query = $data->whereIn('exe_id', $emp_managers)->orderBy('package_id','Desc')->paginate(10);
    ...
    foreach ($teamMembersMapping as $managerId => $members) {
        ...
        $managerTotalQuery = DB::connection('sales_db')->table('payment_history')
            ...
            if ($request->start_date && $request->end_date) {
              ...
              $managerTotalQuery->whereMonth('payment_history.created_date', Carbon::now()->month)->whereYear('payment_history.created_date', Carbon::now()->year);
            }

            if($request->group){
                $managerTotalQuery->where('company_info.group_id',$request->group);
                $active_packages->where('group_id',$request->group);
        ...
        if ($managerTotal) {
            ...
            $detailedQuery = DB::connection('sales_db')->table('payment_history')
             ...
             if ($request->group) {
              ...
              if($request->product) {
                 $detailedQuery->where('payment_history.product_id', $request->product);
...
public function view_manager_team_collection(Request $request)
{
    ...
    if ($request->product) {
        $totalQuery->where('payment_history.product_id', $request->product);
    }
    ...
    foreach ($get_team as $empId) {
        ...
        $active_packages = DB::connection('sales_db')->table('package_info')->where('exe_id',$empId)->where('package_status',1);
        ...
        $totalCollectionArray[$empId] = [
            ...
            'total_retention' => 0,
            'manager' =>$manager_name->emp_fname . ' ' . $manager_name->emp_lame,
            'active_package'=>$active_packages->count()?? '0',
            'details' => []
        ];
    }
    foreach ($totalData as $row) {
        ...
        $manager_name = DB::table('emp_basic_info')->where('emp_id', $empName->reporting_manager)->first();
        $active_packages = DB::connection('sales_db')->table('package_info')->where('exe_id',$row->exe_id)->where('package_status',1);
        ...
        $totalCollectionArray[$row->exe_id] = [
            ...
            'manager'=>$manager_name->emp_fname . ' ' . $manager_name->emp_lame,
            'active_package'=>$active_packages->count(),
            'details' => []
        ];
    }
    $detailedQuery = DB::connection('sales_db')->table('payment_history')
...
public function total_collection(Request $request) {
    ...
    $manager_id = $request->emp_id; 
    $get_team = DB::table('employee_managers')
        ->whereRaw('FIND_IN_SET(?, reporting_to)', [$manager_id])
        ->where('dept_id', 3)
        ->where('status', 1)
        ->pluck('emp_id')
        ->toArray();
    ...
    $active_packages = DB::connection('sales_db')->table('package_info')->where('package_status',1)->whereIn('exe_id',$get_team);
    ...
    $managerTotal = $managerTotalQuery->first();

    $managerInfo = DB::table('emp_basic_info')->where('emp_id', $manager_id)->first();
    ...
    if ($request->start_date && $request->end_date) {
        ...
        $detailedQuery->whereMonth('payment_history.created_date', Carbon::now()->month)
            ->whereYear('payment_history.created_date', Carbon::now()->year);
    }
    ...
    $detailedData = $detailedQuery->groupBy('company_info.group_id', 'payment_history.product_id')->get();

        $total_collection[] = [
            'manager_id' => $manager_id,
            'manager_name' => $managerInfo->emp_fname . ' ' . $managerInfo->emp_lame,
            'total_paid' => $managerTotal->total_paid ?? 0,
            'total_new' => $managerTotal->total_new ?? 0,
            'total_renew' => $managerTotal->total_renew ?? 0,
            'total_due' => $managerTotal->total_due ?? 0,
            'total_retention' => $managerTotal->total_retention ?? 0,
            'active_package'=>$active_packages->count(),
            'details' => $detailedData,
        ];

        return response()->json(['status'=>200,'data'=>$total_collection,'team_exists'=>$team_exists]);

}

public function download_manager_wise_collection(Request $request)
{
    $get_team = [];

     if($request->type =='managercollection'){
         $get_team = DB::table('employee_managers')
            ->whereRaw('FIND_IN_SET(?, reporting_to)', [$request->manager])
            ->where('dept_id', 3)
            ->where('status', 1)
     ...
     $get_team = array_unique($get_team);
      ...
      $empInfo = DB::table('emp_basic_info')
        ->where('emp_id', $request->manager)
        ->first();
    //return  $empInfo;

      $managerinfo =  DB::table('emp_basic_info')
        ->where('emp_id', $empInfo->reporting_manager)
        ->first(['emp_fname', 'emp_lame']);

    $empName = $empInfo->emp_fname . ' ' . $empInfo->emp_lame;
    $managerName = $managerinfo->emp_fname . ' ' . $managerinfo->emp_lame;
    ...
    $detailedQuery->whereMonth('payment_history.created_date', Carbon::now()->month)->whereYear('payment_history.created_date', Carbon::now()->year);
     ...
     $detailedData = $detailedQuery->groupBy('group_names.group_id', 'group_names.name','payment_history.product_id')->get();

     $exportData = [];
    foreach ($detailedData as $detail) {
        $exportData[] = [
            'emp'=>$empName,
            'Manager' => $managerName,
            'Group' => $detail->group_name,
            'Product' => $detail->product_name,
            'New' => $detail->total_new,
            'Renew' => $detail->total_renew,
            'Due' => $detail->total_due,
            'Retention' => $detail->total_retention,
            'Total Collection' => $detail->total_paid,
        ];
    }
...
public function show_manager_wise_followup(Request $request){
    ...
    foreach ($teamMembersMapping as $managerId => $members) {
        ...
        // Main Total Query - Adding `no_of_client_match` using a subquery
        $managerTotalQuery = DB::connection('sales_db')->table('clients_followup_log')
                            ...
                            ->leftJoin('package_info', function($join) {
                            $join->on('package_info.client_id', '=', 'clients_followup_log.client_id')
                            ->on('package_info.comp_id', '=', 'clients_followup_log.comp_id')
                             ->whereRaw('DATE(clients_followup_log.next_followup_date) <= DATE(package_info.created_date)');
                               })
                            ->select(
                                DB::raw("'{$managerId}' as manager_id"),
                                DB::raw("'{$managerName}' as manager_name"),
                                ...
                                DB::raw("COUNT(DISTINCT package_info.client_id) as matched_count")
        ...
        if ($request->start_date && $request->end_date) {
            ...
            $managerTotalQuery->whereDate('clients_followup_log.next_followup_date', Carbon::now()->format('Y-m-d'));
        }
        ...
        if (!$managerTotal) {
            $managerTotal = (object)[
                ...
                'total_paid' => 0,
                'id_count' => 0,
                'matched_count' => 0,  // Default to 0 if no result
            ];
        }

        if ($managerTotal) {
            // Detailed Query (No `no_of_client_match` in this query)
            ...
            if ($request->start_date && $request->end_date) {
                ...
                $detailedQuery->whereDate('clients_followup_log.next_followup_date', Carbon::now()->format('Y-m-d'));
            }
            ...
            // Add the results for each manager
            $results[] = [
                'manager_id' => $managerId,
                'manager_name' => $managerName,
                'total' => $managerTotal,
...
public function show_team_wise_payments_followups(Request $request)
{
    ...
    if ($request->start_date && $request->end_date) {
        ...
        $totalQuery->whereDate('clients_followup_log.next_followup_date', Carbon::now()->format('Y-m-d'));
    }
...
public function package_inactive_notifications(Request $request) {
    ...
    $inactiveCounts = DB::connection('sales_db')->table('package_info')
        ->where(function($query) use ($startOfCurrentWeek, $endOfCurrentWeek, $startOfPreviousWeek, $endOfPreviousWeek) {
            $query->whereBetween('package_end_date', [$startOfCurrentWeek, $endOfCurrentWeek])
...
public function seen_status_of_payment_approval(Request $request){
    ...
    ->where('type',$request->type)->where('client_id',$request->client_id)->update(['seen_status'=>1]);
    return response()->json(['status'=>200]);
}
...
public function recent_inactive_packages(Request $request){
    ...
    if ($request->category) {
     ...
     $total_data =  $data->whereIn('exe_id', $emp_managers)->orderBy('package_end_date', 'desc')->paginate(10);
 ...
 {
    // return DB::table('emp_notifications')->get();
     ...
     if ($request->start_date && $request->end_date) {
        $query->whereDate('cfl.next_followup_date', '>=', $request->start_date)
            ->whereDate('cfl.next_followup_date', '<=', $request->end_date);
     } else {
 ...
 public function sales_upcoming_renewal_data(Request $request)
{
   ...
   $records = DB::connection('sales_db')->table('package_info')
    ...
    public function sales_missing_followup(Request $request)
    {
        ...
        $data = $data->whereIn('clients_followup_log.followup_id', [6, 9, 7, 5]);
        $query = $data->get();
        foreach ($query as $row) {
        ...
        $paginated_data = new \Illuminate\Pagination\LengthAwarePaginator(
            array_slice($data_array, ($request->page - 1) * 10, 10),
            count($data_array),
            10,
            $request->page,
            ['path' => url()->current()]
...
public function followup_details(Request $request) {
    if($request->emp_id == 'RIMS1'){
        $data = DB::table('emp_basic_info')
        ->where('desi_id', 38)
        ->where('emp_status', 1)
    ...
    ->select(
        ...
        DB::raw('SUM(pre_package.package_price) as total_paid'),
        DB::raw("COUNT(DISTINCT clients_followup_log.id) as id_count"),
        DB::raw("COUNT(package_info.client_id) as matched_count")
    )
    ->whereIn('clients_followup_log.id', function ($query) {
        $query->select(DB::raw('MAX(id)'))
            ->from('clients_followup_log')
    ...
    ->whereIn('clients_followup_log.created_by', $members)
    ->whereIn('followup_id', [6, 9, 7, 5])
    ->whereNotExists(function ($query) {
        $query->select(DB::raw(1))
            ->from('package_info')
            ->whereRaw('package_info.client_id = clients_followup_log.client_id')
            ->whereRaw('package_info.comp_id = clients_followup_log.comp_id')
            ->whereRaw('DATE(package_info.created_date) <= DATE(clients_followup_log.next_followup_date) - INTERVAL 10 DAY');
    })
    ->get();


                            //->whereDate('clients_followup_log.next_followup_date', Carbon::now()->format('Y-m-d')); 
         ...
         $payment_without_followups = DB::connection('sales_db')->table('payment_history')
                                   ->leftJoin('clients_followup_log', 'payment_history.comp_id', '=', 'clients_followup_log.comp_id')
                                    ->whereNull('clients_followup_log.comp_id')
        ...
        $noOfDeadFollowup = DB::connection('sales_db')->table('clients_followup_log')
            ->whereIn('clients_followup_log.created_by', $members)
            ->where('clients_followup_log.followup_id', 15);
        ...
        if ($request->start_date && $request->end_date) {
            ...
            $noOfCallConnected->whereDate('clients_followup_log.created_date', '>=', $request->start_date)
            ->whereDate('clients_followup_log.created_date', '<=', $request->end_date);
            $noOfDeadFollowup->whereDate('clients_followup_log.created_date', '>=', $request->start_date)
            ->whereDate('clients_followup_log.created_date', '<=', $request->end_date);

            $payment_without_followups->whereDate('payment_history.created_date', '>=', $request->start_date)
            ->whereDate('payment_history.created_date', '<=', $request->end_date);
        }
        else{
            $managerTotalQuery->whereDate('clients_followup_log.next_followup_date',Carbon::now()->format('Y-m-d'));
            ...
            $noOfDeadFollowup->whereDate('clients_followup_log.created_date',Carbon::now()->format('Y-m-d'));

             $payment_without_followups->whereMonth('payment_history.created_date',Carbon::now()->month);
        ...
        if ($request->group) {
            $managerTotalQuery->where('clients_followup_log.city_id', $request->group);
            $noOfCallConnected->where('clients_followup_log.city_id', $request->group);
            $noOfDeadFollowup->where('clients_followup_log.city_id', $request->group);
        }

        if ($request->product) {
            $managerTotalQuery->where('clients_followup_log.product_id', $request->product);
            $noOfCallConnected->where('clients_followup_log.product_id', $request->group);
            $noOfDeadFollowup->where('clients_followup_log.product_id', $request->group);
        }

        if ($request->category) {
            $managerTotalQuery->where('clients_followup_log.category_id', $request->category);

             $noOfCallConnected->where('clients_followup_log.category_id', $request->group);
            $noOfDeadFollowup->where('clients_followup_log.category_id', $request->group);
        }
        ...
        if (!$managerTotal) {
            $managerTotal = (object)[
                ...
                'id_count' => 0,
                'matched_count' => 0,
                'no_of_call_connected' => 0,
                'no_of_dead_followup' => 0, 
                //'maturity_percent' =>0,
                'dead_percent'=>0,
                'accuracy_percent'=>0,
                'payment_without_followups'=>$payment_without_followups->sum('paid_amount')
            ];
        }

        $managerTotal->no_of_call_connected = $noOfCallConnected->count();
        $managerTotal->no_of_dead_followup = $noOfDeadFollowup->count();
        $managerTotal->payment_without_followups = $payment_without_followups->sum('paid_amount');

        if ($managerTotal->id_count > 0 && $managerTotal->matched_count >0) {
            $managerTotal->accuracy_percent = round(($managerTotal->matched_count / $managerTotal->id_count) * 100);
        } else {
            $managerTotal->accuracy_percent = 0; 
        }

        if ($managerTotal->no_of_dead_followup >0 && $managerTotal->no_of_call_connected > 0 ) {
            $managerTotal->dead_percent = round(($managerTotal->no_of_dead_followup / $managerTotal->no_of_call_connected) * 100);
        } else {
            $managerTotal->dead_percent = 0; 
        }
        ...
        if ($managerTotal) {
            ...
            $detailedQuery = DB::connection('sales_db')->table('clients_followup_log')
                ...
                ->select(
                    'product.product_name',
                    'followup_status.activity_name as followup_name',
                     DB::raw('SUM(pre_package.package_price) as total_paid'),
                    DB::raw('COUNT(clients_followup_log.id) as id_count'),
                    DB::raw("COUNT(DISTINCT package_info.client_id) as matched_count"),
                    DB::raw("CASE WHEN COUNT(DISTINCT package_info.client_id) > 0 THEN ROUND(COUNT(DISTINCT package_info.client_id) / COUNT(clients_followup_log.id)*100) ELSE 0 END as accuracy")
                )
                ->whereIn('clients_followup_log.created_by', $members)
                ->whereIn('followup_id', [6, 9,7,5]);
                //->whereDate('clients_followup_log.next_followup_date', Carbon::now()->format('Y-m-d')); 
        ...
        if ($request->category) {
            ...
            $detailedQuery = $detailedQuery->groupBy('clients_followup_log.product_id', 'clients_followup_log.followup_id')
                                           ->get();
...
public function team_followup_details(Request $request) {
    $data_array = [];
    $emp_id = $request->emp_id;

    // Fetch team members reporting to the given employee
    $get_team = DB::table('employee_managers')
    ...
    // Followup counts with filters
    $followupCounts = DB::connection('sales_db')->table('clients_followup_log')
        ->select(
            'created_by',
            DB::raw("SUM(CASE WHEN disposition = 'connected' AND followup_id NOT IN (24,25) THEN 1 ELSE 0 END) as call_connected"),
            DB::raw("SUM(CASE WHEN followup_id = 15 THEN 1 ELSE 0 END) as dead_followup")
        )
        ->whereIn('created_by', $get_team)
        ->groupBy('created_by');

    // Apply the same filters to followupCounts
    if ($request->start_date && $request->end_date) {
        $followupCounts->whereDate('created_date', '>=', $request->start_date)
            ->whereDate('created_date', '<=', $request->end_date);
    } else {
        $followupCounts->whereDate('created_date', Carbon::now()->format('Y-m-d'));
    }

    if ($request->group) {
        $followupCounts->where('city_id', $request->group);
    }
    if ($request->product) {
        $followupCounts->where('product_id', $request->product);
    }
    if ($request->category) {
        $followupCounts->where('category_id', $request->category);
    }

    $followupCounts = $followupCounts->get()->keyBy('created_by');

    // Fetch manager totals
    $managerTotal = $managerTotalQuery->groupBy('clients_followup_log.created_by')->get();

    if ($managerTotal->isEmpty()) {
        $data_array[] = [
            'emp_name' => ' ',
            'manager' => ' ',
            'total_followups' => 0,
    ...
    // Process manager totals
    foreach ($managerTotal as $row) {
        $empName = DB::table('emp_basic_info')->where('emp_id', $row->created_by)->first();
        $managerName = DB::table('emp_basic_info')->where('emp_id', $empName->reporting_manager)->first();

        $call_connected = $followupCounts[$row->created_by]->call_connected ?? 0;
        $dead_followup = $followupCounts[$row->created_by]->dead_followup ?? 0;

        $mature_percent = $row->id_count > 0 ? ($row->matched_count / $row->id_count) * 100 : 0;
        $dead_percent = $dead_followup > 0 ? ($dead_followup /$call_connected) * 100 : 0;
        ...
        $detailedQuery = DB::connection('sales_db')->table('clients_followup_log')
            ...
            ->where('clients_followup_log.created_by', $row->created_by)
            ->whereIn('clients_followup_log.followup_id', [6, 9, 7, 5]);
        ...
        $detailedData = $detailedQuery->groupBy('clients_followup_log.product_id', 'clients_followup_log.followup_id')->get();

        $data_array[] = [
            'emp_name' => $empName->emp_fname . ' ' . $empName->emp_lame,
            'manager' => $managerName->emp_fname . ' ' . $managerName->emp_lame,
            'total_followups' => $row->id_count,
            'amount' => $row->total_paid,
            'mature_followup' => $row->matched_count,
            'dead_followup' => $dead_followup,
            'no_of_call_connected' => $call_connected,
            'mature_percent' => round($mature_percent),
            'dead_percent' => round($dead_percent),
            'manager_id' => $managerName->emp_id,
            'emp_id' => $row->created_by,
...
public function buffer_amount_details(Request $request){
    ...
    if ($request->manager && !$request->employee) {
       ...
       $data = DB::connection('sales_db')->table('wallet_info')
           ...
           ->where('wallet_info.balance_amount','>',0)
           ->whereIn('client_info.exe_id',$emp_managers)
           ->orderBy('wallet_info.balance_amount', 'DESC');
    ...
    //           ->whereYear('wallet_info.created_date',Carbon::now()->year);
    ...
    //    }
      ...
      public function count_active_packages_with_category_details(Request $request){
        $emp_managers = [];
        if ($request->manager && !$request->selected_emp) {
          $emp_managers = DB::table('employee_managers')
            ->whereRaw('FIND_IN_SET(?, reporting_to)', [$request->manager])
            ->where('status', 1)
            ->where('dept_id', 3)
          ...
          $emp_managers[] = $request->manager;
          $emp_managers = array_unique($emp_managers);
        } elseif ($request->selected_emp) {
          $emp_managers = is_array($request->selected_emp) ? $request->selected_emp : [$request->selected_emp];
        }
        else{
            $emp_managers = DB::table('employee_managers')
            ->whereRaw('FIND_IN_SET(?, reporting_to)', [$request->emp_id])
        ...
        $get_category_details = DB::connection('sales_db')->table('product_category')
          ...
          if(empty($emp_managers)){
            $total_active_packages = $data->where('package_status',1)->count();
