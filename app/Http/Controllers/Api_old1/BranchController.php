...
class BranchController extends Controller
{
  public function save_branch(Request $request){
    ...
    $validator = Validator::make($input, [
      ...
      $branch->branch_number = $request->phone;
      $branch->save();
      if($branch){
 ...
 public function branch_list(){
   // $branch_type = BranchType::get(['id','branch_type','status']);
       $branch = Branch::join('country','country.id','=','branch_details.country_id')
            ...
            ->select('country.name','state.state_name','company_details.business_name',
            'branch_type.branch_type','branch_details.branch_name','branch_details.zip_id',
            'branch_details.full_address','branch_details.status','branch_details.id','branch_details.branch_email','branch_details.branch_number')
            ->get();
...
public function get_active_branch(){
    ...
    return response()->json(['status'=>200,'message'=>'branch status','data'=> $branch_list]);
}
}
