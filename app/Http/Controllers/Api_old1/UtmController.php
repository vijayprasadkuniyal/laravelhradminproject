...
class UtmController extends Controller
{
    public function save_utm(Request $request){
        ...
        public function add_enq_followup(Request $request){
            ...
            $data = array('product_id'=>$request->product,
            'service_id'=>$request->service,'name'=>$request->name,
            'created_by'=>$request->emp_id);
        ...
        public function enq_followup_list(){
            ...
            ->join('product','enq_followup_status.product_id','product.id')
            ->join('product_service','enq_followup_status.service_id','product_service.id')
            ->select('enq_followup_status.id','enq_followup_status.status',
            'enq_followup_status.name','product.product_name','product_service.service_name')
            ->get();
            return response()->json(['status'=>200,'data'=>$data]);
        }
