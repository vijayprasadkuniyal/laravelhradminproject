...

{
    public function add_stock(Request $request){
       ...
       public function stock_list(Request $request){
        ...
        return response()->json(['status'=>200,'message'=>'Stock Details','data'=>$stock_list,'last_page' =>$stock_list->lastPage()]);
       }
  ...
  public function active_vendor(){
    ...
    //public function get_active_stock(){
     ...
     //}
    public function category_based_stock($id){
        $stock_list = Stock::where('status',1)->where('category_id',$id)->where('remaining_quantity', '<>', 0)->get(['id','brand_name']);
        return response()->json(['status'=>200,'message'=>'Avalible Stock','data'=>$stock_list]);
     }
    public function department_employee($id){
        $data = BasicInfo::where('emp_status',1)->where('dept_id',$id)->get(['emp_id','emp_fname']);
    ...
    public function assign_stock(Request $request){
       ...
       /* if($stock_quantity->remaining_quantity==0){
            return response()->json(["message"=>'Out Of Stock','status'=>201]);

        }
        elseif($stock_quantity->remaining_quantity<$request->quantity){
...
public function return_stock_status($id,$emp_id,$stock_id,$date,$status,$remark,$employee_id){
    ...
    $stock_details = array('assign_to'=>$emp_id,'stock_id'=>$stock_id,'created_by'=>$employee_id,'return_status'=>$return_status);
    DB::table('stock_history_details')->insert($stock_details);
...
public function raise_asset_request_list(){
    $data = RaiseAssetRequest::join('category_details','category_details.id','raise_asset_request.category_id')
          ...
          'raise_asset_request.status','raise_asset_request.admin_remark','raise_asset_request.finance_remark')
          ->get();
...
public function get_asset_repair_report(Request $request){
    ...
    $data = DB::table('asset_repair_details')
        ->select('category_id', 'stock_id', DB::raw('SUM(quantity) as quantity'), DB::raw('SUM(amount) as amount'))
        ->whereBetween('date', [$start_date, $end_date])
        ->groupBy('category_id', 'stock_id')
        ->get();
