...
public function leave_permission($id){
   $leave_blance = LeaveType::where('assign_when_request',1)->where('is_probation_leave',0)->where('status',1)->get();
...
public function assign_leave(){
  ...
  foreach($data as $row){
    if($row->is_confirmed==0){
      ...
      $confirmed_leave = LeaveType::where('is_confirmation_leave',1)->where('assign_when_request',0)->get();
