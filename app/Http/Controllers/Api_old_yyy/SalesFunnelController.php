...
public function sales_dashboard_data(Request $request) {
    ...
    $collection_of_amount = DB::connection('sales_db')->table('payment_history')->leftJoin('client_info','client_info.client_id','payment_history.client_id');

    $package_pending_activation = DB::connection('sales_db')->table('package_info');

    $total_followup = DB::connection('sales_db')->table('clients_followup_log')->leftJoin('client_info','client_info.client_id','clients_followup_log.client_id');

    $forcast_accuracy = DB::connection('sales_db')->table('clients_followup_log')->leftJoin('client_info','client_info.client_id','clients_followup_log.client_id');


    $count_regular_client =  DB::connection('sales_db')->table('package_info');

    $today_collection = DB::connection('sales_db')->table('payment_history')->leftJoin('client_info','client_info.client_id','payment_history.client_id');

    $today_followup = DB::connection('sales_db')->table('clients_followup_log')->leftJoin('client_info','client_info.client_id','clients_followup_log.client_id');

     if($request->group){
         $total_upcoming_renewal->whereRaw('FIND_IN_SET(?, group_id)', [$request->group]);

         $collection_of_amount->where('client_info.group_id',$request->group);

         $total_followup->where('client_info.group_id',$request->group);

         $package_pending_activation->whereRaw('FIND_IN_SET(?, group_id)', [$request->group]);

         $today_collection->where('client_info.group_id',$request->group);

         $today_followup->where('client_info.group_id',$request->group);

         $forcast_accuracy->where('client_info.group_id',$request->group);
    ...
    if($request->start_date && $request->end_date){
         $total_upcoming_renewal->whereDate('package_end_date', '>=',$request->start_date)
                ->whereDate('package_end_date', '<=', $request->end_date);
    ...
    if(!$request->start_date && !$request->end_date){
         $total_upcoming_renewal->whereMonth('package_end_date', Carbon::now()->month);

         $collection_of_amount->whereMonth('created_date', Carbon::now()->month);

         $package_pending_activation->whereMonth('created_date', Carbon::now()->month);

         $total_followup->whereMonth('created_date', Carbon::now()->month);
       ...
       // return 'kk';
        ...
        $total_new_sale = $collection_of_amount->whereIN('created_by',$emp_managers)->where('payment_for','New')->sum('paid_amount');

        $total_renew_sale = $collection_of_amount->whereIN('created_by',$emp_managers)->where('payment_for','Renew')->sum('paid_amount');

        $due_amount =  $collection_of_amount->whereIN('created_by',$emp_managers)->where('payment_for','Due Amount')->sum('paid_amount');
...
$package_activation = $package_pending_activation->whereIn('created_by', $emp_managers)
        ...
        $total_no_of_followup =  $total_followup->whereIn('created_by', $emp_managers)->count();
...
public function send_sales_notification($emp_id){
    ...
    if($data>0){
        ...
        $notification = 'Click To View Yesterday Missing Followup Of Team';
    ...
    if($kra_kp_data>0){
         ...
         $notification = 'Click To View Yesterday Missing Kra Kpi Of Team';
        $data_array = array('emp_id'=>$emp_id,'type'=> $type,'notification'=>$notification);
...
public function missing_kra_kpi_history(Request $request) {
    ...
    foreach ($kra_kpi_history as $row) {
        $emp_name = BasicInfo::where('emp_id', $row->created_by)->first();
        $manager = BasicInfo::where('emp_id', $emp_name->reporting_manager)->first();
    ...
    return response()->json(['status' => 200, 'data' => $details_array]);
}
...
public function show_enquiry_data_on_sales_dashboard(Request $request){
    ...
    $emp_groups = BasicInfo::where('emp_id',$request->emp_id)->first();
    if($emp_groups->assigned_group!=''){
        $group_ids = explode(',',$emp_groups->assigned_group);

     }

     $total_not_sent_in_groups =  DB::connection('sales_db')->table('enquiry_info')->where('enq_status',3);
     $total_not_sent_in_category =  DB::connection('sales_db')->table('enquiry_info')->where('enq_status',3);

     $total_enquiry_lmart = DB::connection('sales_db')->table('enquiry_info')->where('product_id',1);
     $total_enquiry_zoopgo = DB::connection('sales_db')->table('enquiry_info')->where('product_id',2);


   // $get_clients_id = DB::connection('sales_db')->table('client_info')->whereIn('exe_id',$emp_managers)->pluck('client_id');
    //return  $get_clients_id;
   

  $lead_info_data = DB::connection('sales_db')->table('leads_info')->leftJoin('enquiry_info','enquiry_info.id','leads_info.enq_id')
      ->leftJoin('client_info','client_info.client_id','leads_info.client_id')
      ->whereIn('client_info.exe_id',$emp_managers );



   if($request->product){
     $lead_info_data->where('leads_info.product_id',$request->product);
     //$total_enquiry_lmart->where('product_id',$request->product);
   }
   if($request->category){
     $lead_info_data->where('leads_info.category_id',$request->category);
     $total_not_sent_in_category->where('category_id',$request->category);
     $total_enquiry_lmart->where('category_id',$request->category);
     $total_enquiry_zoopgo->where('category_id',$request->category);

   }
   if($request->service){
     $lead_info_data->where('leads_info.service_id',$request->service);
      $total_enquiry_lmart->where('service_id',$request->service);
      $total_enquiry_zoopgo->where('service_id',$request->service);

   }
    if($request->group){
      $lead_info_data->where('enquiry_info.group_id',$request->group);
      $total_not_sent_in_groups->where('group_id',$request->group);
      $total_enquiry_lmart->where('group_id',$request->group);
       $total_enquiry_zoopgo->where('group_id',$request->group);
     }
    else{
         $total_not_sent_in_groups->whereIn('group_id',$group_ids);
         $total_enquiry_lmart->whereIn('group_id',$group_ids);
          $total_enquiry_zoopgo->whereIn('group_id',$group_ids);


    }
    if($request->start_date && $request->end_date){
       $lead_info_data->whereDate('leads_info.created_date', '>=',$request->start_date)
                ->whereDate('leads_info.created_date', '<=',$request->end_date);
      $total_not_sent_in_groups->whereDate('created_date', '>=',$request->start_date)
                ->whereDate('created_date', '<=',$request->end_date);
      $total_not_sent_in_category->whereDate('created_date', '>=',$request->start_date)
                ->whereDate('created_date', '<=',$request->end_date);
      $total_enquiry_lmart->whereDate('created_date', '>=',$request->start_date)
                ->whereDate('created_date', '<=',$request->end_date);
     $total_enquiry_zoopgo->whereDate('created_date', '>=',$request->start_date)
                ->whereDate('created_date', '<=',$request->end_date);

    }
    else{

       $lead_info_data->whereMonth('leads_info.created_date', $currentmonth)
            ->whereYear('leads_info.created_date',$currentyear);
        $total_not_sent_in_groups->whereMonth('created_date', $currentmonth)
            ->whereYear('created_date',$currentyear);
        $total_not_sent_in_category->whereMonth('created_date', $currentmonth)
            ->whereYear('created_date',$currentyear);
        $total_enquiry_lmart->whereMonth('created_date', $currentmonth)
            ->whereYear('created_date',$currentyear);
        $total_enquiry_zoopgo->whereMonth('created_date', $currentmonth)
            ->whereYear('created_date',$currentyear);

    }
    $total_count = $lead_info_data->count();
    $total_lead_price = $lead_info_data->sum('lead_value');
    if($total_count>0){
        $sale_price =  $total_lead_price/ $total_count;
    }
    else{
        $sale_price = 0;
    }

    $lead_sale =  $lead_info_data->count();
    $group_loss = $total_not_sent_in_groups->count();
    $category_loss = $total_not_sent_in_category->count();
    $total_enquiry_count_lmart = $total_enquiry_lmart->count();
    $total_sent_enquiry_lmart= $total_enquiry_lmart->where('enq_status',1)->count();
    $total_enquiry_count_zoopgo = $total_enquiry_zoopgo->count();
    $total_sent_enquiry_zoopgo= $total_enquiry_zoopgo->where('enq_status',1)->count();

    if($total_enquiry_count_lmart>0){
        $sent_percent_lmart = round($total_sent_enquiry_lmart/$total_enquiry_count_lmart*100);
    }
    else{
    ...
    if($total_enquiry_count_zoopgo>0){
         $sent_percent_zoopgo = round($total_sent_enquiry_zoopgo/$total_enquiry_count_zoopgo*100);
    ...
    $data_array = array('sale_price'=>$sale_price,'lead_sale'=>$lead_sale,'group_loss'=> $group_loss,'category_loss'=>$category_loss,'lmart_sent_percent'=>$sent_percent_lmart,
        'sent_percent_zoopgo'=>$sent_percent_zoopgo);
