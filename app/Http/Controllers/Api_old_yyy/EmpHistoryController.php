...
public function save_resign_description_details(Request $request){
  ...
  public function pip_task_saved(Request $request){
    $data = array('emp_id'=>$request->selectedemployee,
      'date_from'=>$request->start_date,'date_to'=>$request->end_date,
      'task_details'=>$request->task,'reason_of_pip'=>$request->reason,
      'created_by'=>$request->emp_id,'department_id'=>$request->department);

      DB::table('employee_pip')->insert($data);

      return response()->json(['status'=>200,'message'=>'Task Assigned Successfully']);

  }

  public function pip_list(){
    $data = DB::table('employee_pip')->join('emp_basic_info',
             'emp_basic_info.emp_id','employee_pip.emp_id')
            ->join('department','department.id','employee_pip.department_id')
            //->join('emp_basic_info.emp_id','employee_pip.created_by')
            ->select('department.department_name','employee_pip.date_from','employee_pip.date_to','employee_pip.task_details','employee_pip.reason_of_pip','employee_pip.id','emp_basic_info.emp_fname','emp_lame','employee_pip.approval_status')
            ->orderBy('id','DESC')
            ->get();
  ...
  public function view_pip_description($id){
    $data = DB::table('employee_pip')->where('id',$id)->first();
    return response()->json(['status'=>200,'task'=> $data->task_details,
      'reason'=>$data->reason_of_pip]);
  }

  public function pip_task_status_update(Request $request){
    $data = array('approval_status'=>$request->status,'approval_remark'=>$request->remark);
    DB::table('employee_pip')->where('id',$request->id)->update($data);
    return response()->json(['status'=>200,'message'=>'status Updated Successfully']);
  }

  public function emp_pip_task($id){
       $data = DB::table('employee_pip')->join('emp_basic_info',
             'emp_basic_info.emp_id','employee_pip.emp_id')
            //->join('emp_basic_info.emp_id','employee_pip.created_by')
            ->select('employee_pip.date_from','employee_pip.date_to','employee_pip.task_details','employee_pip.reason_of_pip','employee_pip.id','emp_basic_info.emp_fname','emp_lame','employee_pip.approval_status')
            ->orderBy('id','DESC')
            ->where('employee_pip.emp_id',$id)
            ->get();
