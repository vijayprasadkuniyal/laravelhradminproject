...
public function get_sales_followup_data(Request $request){
    ...
    //return  $curr_date;
    $data = DB::connection('sales_db')->table('clients_followup_log')->select('product_id',
    'service_id','followup_id','created_by','client_id')->groupBy('product_id',
    'service_id','followup_id','created_by','client_id')->get();
    foreach($data as $row){
     $calc_data = DB::connection('sales_db')->table('clients_followup_log')->whereDate('created_date',$curr_date)
     ->where('product_id',$row->product_id)->where('service_id',$row->service_id)
     ->where('followup_id',$row->followup_id)
     ->where('created_by',$row->created_by)->where('client_id',$row->client_id);
     $get_emp_manager = DB::table('employee_managers')->where('emp_id',$row->created_by)->where('status',1)->first();
     $total_left_followup =  $calc_data->where('next_followup_date', $curr_date)->get();
     return $total_left_followup ;
     foreach($total_left_followup as $followp){


      }
    
     $list[]  = array('product_id'=>$row->product_id,'service_id'=>$row->service_id,
    'task_id'=>$row->followup_id,'no_of_task'=>$calc_data->count(),
    'amount'=>$calc_data->sum('amount'),'created_by'=>$row->created_by,
    'reporting_manager'=>$get_emp_manager->reporting_to,'client_id'=>$row->client_id);
    

    }
    DB::connection('sales_db')->table('sales_funnel')->insert($list);
...
public function count_sales_data(Request $request){
   if($request->dept_id==3){
    $check_reporting_manager = DB::connection('sales_db')->table('sales_funnel')->whereRaw('FIND_IN_SET(?, reporting_manager)', [$request->emp_id])->exists();
    
    if($check_reporting_manager){
     $payment_for_followup = DB::connection('sales_db')->table('sales_funnel')->where('task_id',6)->whereRaw('FIND_IN_SET(?, reporting_manager)', [$request->emp_id]);
     $meeting_fix = DB::connection('sales_db')->table('sales_funnel')->where('task_id',7)->whereRaw('FIND_IN_SET(?, reporting_manager)', [$request->emp_id]);
     $no_of_call = DB::connection('sales_db')->table('sales_funnel')->whereRaw('FIND_IN_SET(?, reporting_manager)', [$request->emp_id]);
     $proposal_send = DB::connection('sales_db')->table('sales_funnel')->where('task_id',7)->whereRaw('FIND_IN_SET(?, reporting_manager)', [$request->emp_id]);
     $won = DB::connection('sales_db')->table('sales_funnel')->where('task_id',19)->whereRaw('FIND_IN_SET(?, reporting_manager)', [$request->emp_id]);
     $loss = DB::connection('sales_db')->table('sales_funnel')->where('task_id',15)->whereRaw('FIND_IN_SET(?, reporting_manager)', [$request->emp_id]);
     $amount = DB::connection('sales_db')->table('sales_funnel')->where('task_id','!=',18)->whereRaw('FIND_IN_SET(?, reporting_manager)', [$request->emp_id]);

    if($request->product){
         $payment_for_followup->where('product_id',$request->product);
         $meeting_fix->where('product_id',$request->product);
         $no_of_call->where('product_id',$request->product);
         $proposal_send->where('product_id',$request->product);
         $won->where('product_id',$request->product);
         $loss->where('product_id',$request->product);
         $amount->where('product_id',$request->product);

    }
    if($request->category){
        $payment_for_followup->where('service_id',$request->category);
        $meeting_fix->where('service_id',$request->category);
        $no_of_call->where('service_id',$request->category);
        $proposal_send->where('service_id',$request->category);
        $won->where('service_id',$request->category);
        $loss->where('service_id',$request->category);
        $amount->where('service_id',$request->category);

    }
    if($request->start_date && $request->end_date){
        $payment_for_followup->whereBetween('created_date', [$request->start_date, $request->end_date]);
        $meeting_fix->whereBetween('created_date', [$request->start_date, $request->end_date]);
        $no_of_call->whereBetween('created_date', [$request->start_date, $request->end_date]);
        $proposal_send->whereBetween('created_date', [$request->start_date, $request->end_date]);
        $won->whereBetween('created_date', [$request->start_date, $request->end_date]);
        $loss->whereBetween('created_date', [$request->start_date, $request->end_date]);
        $amount->whereBetween('created_date', [$request->start_date, $request->end_date]);
    }
    if(!$request->start_date && !$request->end_date){
        //$data = $list_data->whereMonth('created_date', Carbon::now()->month)->get();
        $payment = $payment_for_followup->whereMonth('created_date', Carbon::now()->month)->count();
        $meeting =  $meeting_fix->whereMonth('created_date', Carbon::now()->month)->count();
        $call = $no_of_call->whereMonth('created_date', Carbon::now()->month)->count();
        $proposal = $proposal_send->whereMonth('created_date', Carbon::now()->month)->count();
        $total_won = $won->whereMonth('created_date', Carbon::now()->month)->count();
        $total_loss = $loss->whereMonth('created_date', Carbon::now()->month)->count();
        $total_amount = $amount->whereMonth('created_date', Carbon::now()->month)->sum('amount');

    }
    else{
        $payment = $payment_for_followup->count();
        $meeting = $meeting_fix->count();
        $call = $no_of_call->count();
        $proposal = $proposal_send->count();
        $total_won =  $won->count();
        $total_loss = $loss->count();
        $total_amount =  $amount->sum('amount');

    }
    $list_of_data = array('payment_for_followup'=>$payment,'meeting_fix'=>$meeting,'no_of_call'=>$call,
    'proposal_send'=>$proposal,'won'=>$total_won,'loss'=> $total_loss,'amount'=>$total_amount);
    //return $list_of_data;
    
    }
    else{
     $payment_for_followup = DB::connection('sales_db')->table('sales_funnel')->where('task_id',6)->where('created_by',$request->emp_id);
     $meeting_fix = DB::connection('sales_db')->table('sales_funnel')->where('task_id',7)->where('created_by',$request->emp_id);
     $no_of_call = DB::connection('sales_db')->table('sales_funnel')->where('created_by',$request->emp_id);
     $proposal_send = DB::connection('sales_db')->table('sales_funnel')->where('task_id',7)->where('created_by',$request->emp_id);
     $won = DB::connection('sales_db')->table('sales_funnel')->where('task_id',19)->where('created_by',$request->emp_id);
     $loss = DB::connection('sales_db')->table('sales_funnel')->where('task_id',15)->where('created_by',$request->emp_id);
     $amount = DB::connection('sales_db')->table('sales_funnel')->where('created_by',$request->emp_id);
    ...
    return response()->json(['status' => 200, 'data' =>$list_of_data]);
}
