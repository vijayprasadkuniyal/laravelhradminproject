...
class LeaveTypeController extends Controller
{
     ...
     // return $request->all();
      ...
      $leave->is_probation_leave = $request->probation_leave;
       $leave->is_assign_quartly = $request->is_assign_quartly;
...
public function get_active_leavetype($id){
  //return 'jj';
    ...
    ->where(function ($query) {
        $query->where('quarter_no', Carbon::now()->quarter)
              ->orWhereNull('quarter_no');
    })
    ->distinct()
    ->get();
...
public function get_active_confirmation_leave(){
    $leave_type = LeaveType::where('is_probation_leave',1)->where('assign_when_request',0)->where('is_assign_quartly',1)->where('status',1)
               ->get();
    return response()->json(['status' => 200, 'message' => 'Leave details','data'=>$leave_type]);
