...
class TicketRiseController extends Controller
{
    public function ticket_rise(Request $request){
            ...
            if($request->department_to==3 && $request->group!=''){
                $group = $request->group;
                $emp_details = DB::table('emp_basic_info')->where('dept_id',3)->where('desi_id',38)->where('emp_status',1)->WhereRaw('FIND_IN_SET(?,assigned_group)', [$group])->first();
                $emp_id =  $emp_details->emp_id;
               // return $emp_id;
            }
            else{
               $emp_details = DB::table('emp_basic_info')->where('dept_id',$request->department_to)->whereIn('desi_id',[1,15,26,31,10])->where('emp_status',1)->first();
                $emp_id = $emp_details->emp_id;
            ...
            'description'=>$request->description,'created_by'=>$request->emp_id);
             ...
             $type="ticket_raise";
             //$action_by = 
             $notification_data = array('emp_id'=>$emp_id,'notification'=>$message,'type'=>$type,'action_by'=>$raise_by->emp_fname,'ticket_id'=>$ticket_id);
             DB::table('emp_notifications')->insert($notification_data);
             //return $emp_id;
              event(new TicketEvent($message,$emp_id,$type,$ticket_id));
  ...
  public function ticket_receive_list(Request $request, $id)
{
    ...
    $data = BasicInfo::where('reporting_manager', $id)->where('emp_status',1)->first();
...
public function get_assign_ticket_list($id){
   ...
   public function get_ticket_details($id){
    ...
    return response()->json(['status'=>200,'message'=>'Ticket Details','data'=>$data,'assign'=>$assignedMemberNames,'assign_array'=>$assignids,'assign_from'=>$assign_from,'raise_by'=>$ticket_raise_by->created_by,'product'=>$products,'ticket_raise_by'=>$emp_name->emp_fname,'new_time'=>$time]);
