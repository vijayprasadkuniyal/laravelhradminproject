     ...
     $validator = Validator::make($input, [
      ...
      // return $data;
       ...
       if(!empty($row->amount)){
        ...
        // $inserted_data = Sallary::where('emp_id',$request->emp_id)->get();

        // $history_data = array('emp_id'=>$request->emp_id,'salary_table_id'=>$inserted_data->id,'attribute_id'=>$row->attribute_id,'amount_per_anum'=>$row->amount,'amount_per_month'=>$row->per_month,'status'=>1,'emp_package'=>$request->package_detail,'tds'=>$request->tds);
        // DB::table('emp_salary_history')->insert($history_data);
      
      }
      $last_inserted_data = Sallary::where('emp_id',$request->emp_id)->get();
      foreach($last_inserted_data as $last){
        $inserted_data[] = array('emp_id'=>$request->emp_id,
        'salary_table_id'=>$last->id,'attribute_id'=>$last->attribute_id,
         'amount_per_anum'=>$last->amount_per_anum,'amount_per_month'=>$last->amount_per_month,
         'status'=>1,'emp_package'=>$last->emp_package,'is_deduction'=>$last->is_deduction,'tds'=>$last->tds);

      }
       DB::table('emp_salary_history')->insert($inserted_data);
...
public function salary_calc(Request $request)
{
  //return $checkbox;
    ...
    $salary_attributes = Salaryattribute::where('attribute', '!=', 'TDS')
        ->where('is_checked', 0)
        ->where('status', 1)
        ->get();
