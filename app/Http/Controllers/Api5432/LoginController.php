  ...
  public function logout_emp($id){
     ...
     $emp_details = BasicInfo::where('emp_id',$id)->first();
      ...
      $financialYear = $financialYearStart . '-' . $financialYearEnd;
       ...
       $payment_for_followup =  DB::table('save_company_target')->where('subattribute_id',13)->where('department_id',3)->where('financial_year',$financialYear)->first();
       ...
       $business_proposal =  DB::table('save_company_target')->where('subattribute_id',14)->where('department_id',3)->where('financial_year',$financialYear)->first();
      ...
      $followup_for_meeting =  DB::table('save_company_target')->where('subattribute_id',15)->where('department_id',3)->where('financial_year',$financialYear)->first();
      ...
      $calls_required_data = DB::table('save_company_target')->where('subattribute_id',12)->where('department_id',3)->where('financial_year',$financialYear)->first();
     ...
     if($emp_details->desi_id !=38){
      $get_data = DB::connection('sales_db')->table('sales_funnel')->where('created_by',$id)->whereDate('created_date',Carbon::now()->format('Y-m-d'));
      }
      else{
         $get_data = DB::connection('sales_db')->table('sales_funnel')->whereRaw('FIND_IN_SET(?, reporting_manager)', [$id])->whereDate('created_date',Carbon::now()->format('Y-m-d'));

      }
      $total_calls = (clone $get_data)->sum('no_of_task');
      $total_payment_for_followup = (clone $get_data)->where('task_id', 6)->sum('no_of_task');
      $total_proposal = (clone $get_data)->where('task_id', 5)->sum('no_of_task');
      $total_meeting_fix = (clone $get_data)->where('task_id', 7)->sum('no_of_task');
      ...
      if(!$check_leave && !$check_emp_leave){
...
public  function get_emp_idle_time(Request $request){
  ...
  if($request->type =='break'){
    $data = DB::table('store_employee_break_time')->whereIn('emp_id',$emp_managers)->select('emp_id', DB::raw('DATE(created_date) as created_date'), DB::raw('SUM(TIMESTAMPDIFF(MINUTE, start_time, end_time)) as total_time'))
    ->groupBy('emp_id', DB::raw('DATE(created_date)'));

  }
  else{
     $data = DB::table('store_idle_time')->whereIn('emp_id',$emp_managers)->select('emp_id', DB::raw('DATE(created_date) as created_date'), DB::raw('SUM(TIMESTAMPDIFF(MINUTE, time_start_from, time_end)) as total_time'))
    ->groupBy('emp_id', DB::raw('DATE(created_date)'));
