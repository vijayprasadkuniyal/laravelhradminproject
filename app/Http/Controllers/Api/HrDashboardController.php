...
class HrDashboardController extends Controller
{
   public function hr_dashboard_count(Request $request){
    ...
    $no_of_request = DB::table('leave_request_details')->count();
    $no_of_confirmation_request = DB::table('emp_basic_info')->where('emp_status',1);
    ...
    if($request->start_date && $request->end_date){
        ...
        ->whereDate('created_date', '<=', $request->end_date);
        $no_of_confirmation_request->whereDate('emp_doc', '>=', $request->start_date)
        ->whereDate('emp_doc', '<=', $request->end_date);
      
    }
    else{
        $no_of_confirmation_request->whereMonth('emp_doc',Carbon::now()->month)->whereYear('emp_doc',Carbon::now()->year);
    ...
    'attendance_regularize'=>$attendane_regularize->count(),'job_request'=> $job_request->count(),
    'offer_letter_count'=>$offer_letter->count(),'leave_request'=>$no_of_request,
       'count_of_confirmation_request'=>$no_of_confirmation_request->count());
