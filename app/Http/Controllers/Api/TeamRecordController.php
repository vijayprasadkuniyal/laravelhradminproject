...
if($request->status ==2){
  if( $emp_leave_blance->leave_taken>0){
   ...
   $data = EmployeeLeave::where('id',$request->leaveId)->where('emp_id',$request->empId)->
   update(['status'=>$request->status,'rejection_reason'=>$request->rejectionReason,'action_by'=>$request->emp_id]);
...
{
    // Get the list of manager IDs based on the emp_id provided
    ...
    // Query to fetch work-from-home data
    $data = BasicInfo::join('work_from_home', 'work_from_home.employee_id', '=', 'emp_basic_info.emp_id')
        ->leftJoin('attendance', function ($join) use ($request) {
            $join->on('attendance.emp_id', '=', 'work_from_home.employee_id')
                 ->where('attendance.attendance_date', '=', DB::raw('work_from_home.days_from'));
        })
        ->select(
            'emp_basic_info.emp_fname',
            'emp_basic_info.emp_mname',
            'emp_basic_info.emp_lame',
            'work_from_home.days_from',
            'work_from_home.days_to',
            'work_from_home.no_of_days',
            'work_from_home.status',
            'work_from_home.rejection_reason',
            'work_from_home.reason_for_wfh',
            'work_from_home.id',
            'work_from_home.employee_id',
            'work_from_home.created_date',
            'attendance.status as attendance_status'
        )
        ->whereIn('work_from_home.employee_id', $managers)
        ->orderBy('work_from_home.id', 'DESC');

    // Filter by employee name if provided
    if ($request->emp_name) {
        $data->where(function ($query) use ($request) {
            $query->where('emp_basic_info.emp_fname', 'LIKE', '%' . $request->emp_name . '%')
                  ->orWhere('emp_basic_info.emp_lame', 'LIKE', '%' . $request->emp_name . '%');
        });
    }
    ...
    // Map attendance status to custom values
    $data->getCollection()->transform(function ($item) {
        switch ($item->attendance_status) {
            ...
            case 2:
                ...
                break;
            case 3:
                $item->attendance_status = 'Short Leave';
                break;
            default:
                $item->attendance_status = 'Not Available';
                break;
        }
        return $item;
    });
    ...
    return response()->json([
        'status' => 200,
        'message' => 'Work From Home Details',
        'data' => $data,
   ...
   // return $request->all();
     ...
     WFH::where('id',$request->id)->update(['status'=>$request->status,
        'rejection_reason'=>$request->rejectionReason,'action_by'=>$request->action_by]);
