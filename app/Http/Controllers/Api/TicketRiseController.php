...
class TicketRiseController extends Controller
{
    public function ticket_rise(Request $request){
            ...
            'service_id'=>$request->service,'category_id'=>$request->category,
            ...
            $group_array = explode(',', $groups);
                ...
                $ticket_data = DB::table('ticket_rise')
                    ...
                    ->join('department','ticket_rise.request_from_dept','=','department.id',)
                     ->join('ticket_priority','ticket_priority.id','=','ticket_rise.attribute_id',)
                      ->join('ticket_subattribute','ticket_subattribute.id','=','ticket_rise.subattribute_id',)
                       ->select('ticket_rise.id','ticket_rise.ticket_id','ticket_rise.ticket_status','ticket_rise.close_status', 'emp_basic_info.emp_fname as created_by_name',
                       'department.department_name','ticket_priority.attribute_name',
                       'ticket_subattribute.name','ticket_subattribute.action_time',
                       'ticket_subattribute.completion_time','ticket_rise.request_to_dept',
                       'ticket_rise.group_id','ticket_rise.created_by')
                    ->get();
...
public function get_assign_ticket_list($id){
   ...
   public function view_ticket_status($ticket_id,Request $request){
    ...
    $ticket_data =  DB::table('ticket_history')
                    ...
                    ->join('ticket_rise','ticket_rise.ticket_id','ticket_history.ticket_id')
                    ->select('emp_basic_info.emp_fname','ticket_history.ticket_id','ticket_rise.close_status',
    ...
    public function close_ticket(Request $request){
        TicketRise::where('ticket_id',$request->ticket_id)->update(['close_status'=>1,'close_date'=>Carbon::now(),'ticket_status'=>2]);
   ...
   public function get_ticket_details($id){
    ...
    $names = DB::connection('sales_db')->table('group_names')
    ...
    $product = DB::connection('sales_db')->table('product')->where('id',$data->product_id)->first();
    $service = DB::connection('sales_db')->table('product_service')->where('id',$data->service_id)->first();
    $category = DB::connection('sales_db')->table('product_category')->where('id',$data->category_id)->first();
    if($product){
    ...
    $total_time_ticket_active = '';
    if($data->close_status==0){
        $now = Carbon::now();
        $total_time_ticket_active = $now->diffInHours($data->created_date);
      }
    else{
      $total_time_ticket_active = carbon::parse($data->close_date)->diffInHours($data->created_date);

    }

    
    //return $assigned_member;
    $data = array('ticket_status'=>$data->close_status,'group_names'=>$names,'description'=>$data->description,'close_date'=>$data->close_date,'url'=>$data->url,'date' => Carbon::parse($data->created_date)->format('Y-m-d'),'service'=>$service->service_name??'','category'=>$category->category_name??'','ticket_active_time'=>$total_time_ticket_active);
    
    return response()->json(['status'=>200,'message'=>'Ticket Details','data'=>$data,'assign'=>$assignedMemberNames,'assign_array'=>$assignids,'assign_from'=>$assign_from,'raise_by'=>$ticket_raise_by->created_by,'product'=>$products,'ticket_raise_by'=>$emp_name->emp_fname.' '.$emp_name->emp_lame,'new_time'=>$commaSeparatedTimes,]);
...
public function edit_ticket($id){
  $data = DB::table('ticket_rise')->where('ticket_id',$id)->first();
  return response()->json(['status'=>200,'data'=>$data]);
      ...
      public function update_ticket(Request $request){
           ...
           $ticket_details = DB::table('ticket_rise')->where('ticket_id',$request->ticket_id)->first();
          ...
          $data = array('request_from_dept'=>$request->dept_id,'request_to_dept'=>$request->department_to,
            'product_id'=>$request->product,'group_id'=>$group,
            'attribute_id'=>$request->attribute,
            'service_id'=>$request->service,'category_id'=>$request->category,
            'subattribute_id'=>$request->subattribute,'issue_page_url'=>$request->url??'',
            'description'=>$request->description,'created_by'=>$request->emp_id);
             DB::table('ticket_rise')->where('ticket_id',$request->ticket_id)->update($data);

             $message = $raise_by->emp_fname. ' '. 'Raise A Ticket For You Pls See';
             $type="ticket_raise";
             foreach($emp_details as $row){
                $notification_data[] = array('emp_id'=>$row,'notification'=>$message,'type'=>$type,'action_by'=>$raise_by->emp_fname,'ticket_id'=>$request->ticket_id);
                  event(new TicketEvent($message,$row,$type,$request->ticket_id));
             ...
             return response()->json(['status'=>200,'message'=>'Ticket Updated successfully']);
}

public function ticket_notification_seen_status(Request $request) {
  $request->validate([
      'emp_id' => 'required',
      'type' => 'required',
  ]);
  
  DB::table('emp_notifications')
...
public function assign_member_details($ticket_id){
  $data_array = [];
  $data = DB::table('ticket_task_assign')->where('ticket_id',$ticket_id)->get();
  foreach($data as $row){
    $assign_emp_details = DB::table('emp_basic_info')->where('emp_id',$row->assign_to)->first();
    if($row->status ==0){
      $status = 'pending';
   ...
   if($row->ticket_solve_date==''){
    $now = Carbon::now();
    $total_time_ticket = $now->diffInHours($row->created_date);
  }
  else{
    $total_time_ticket = carbon::parse($row->ticket_solve_date)->diffInHours($row->created_date);

 }
 $ticket_raiser_details = DB::table('ticket_rise')->where('ticket_id',$ticket_id)->first();

 $data_array[] = array('id'=>$row->id,'status'=>$status,'assign_date'=>$row->created_date,
              'ticket_solve_date'=>$row->ticket_solve_date,'ticket_id'=>$row->ticket_id,
              'total_active_time_on_ticket'=>$total_time_ticket,
              'emp_name'=>$assign_emp_details->emp_fname.' '.$assign_emp_details->emp_lame,'ticket_rise_by'=>$ticket_raiser_details->created_by);
...
public function update_assigned_member_task(Request $request){
     $data = array('status'=>$request->status,'ticket_solve_date'=>$request->task_date);
     DB::table('ticket_task_assign')->where('id',$request->id)->update($data);
     return response()->json(['status'=>200,'message'=>'Updated Successfully']);
