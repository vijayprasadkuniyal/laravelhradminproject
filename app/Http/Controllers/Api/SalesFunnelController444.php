...
public function get_followups_details(Request $request) {
    ...
    $data = DB::connection('sales_db')->table('clients_followup_log')
        ->leftJoin('client_info', 'client_info.client_id', 'clients_followup_log.client_id')
        ->leftJoin('guest_user_details', 'guest_user_details.id', 'clients_followup_log.client_id')
        ->select('clients_followup_log.*', 'client_info.client_name', 'client_info.group_id as client_info_group', 'guest_user_details.group_id as guest_group');
        ...
        $data->where(function ($query) use ($request) {
            $query->where(function ($subQuery) use ($request) {
                ...
                $subQuery->where('clients_followup_log.client_type', 2)
                    ->where('client_info.group_id', $request->group);
            });
        });
    }
    ...
    foreach ($query as $row) {
        // Fetch related data based on client_type
        ...
        $total_no_of_pkg_buys = DB::connection('sales_db')->table('package_info')->where('client_id', $row->client_id)->count();
        $last_package_by_date = DB::connection('sales_db')->table('package_info')->where('client_id', $row->client_id)->orderBy('package_id', 'DESC')->first();
        
        $pkg_buy_date = $last_package_by_date ? $last_package_by_date->package_start_date : '';

        if ($row->refer_package_id != '') {
            $pkg_details = DB::connection('sales_db')->table('package_info')->where('package_id', $row->refer_package_id)->first();
            $pkg_amount = $pkg_details->package_amount;
...
public function carry_forward_followups(Request $request){
    ...
    $data = DB::connection('sales_db')->table('clients_followup_log')->leftJoin('client_info','client_info.client_id','clients_followup_log.client_id')
       ->select('client_info.client_name','client_info.group_id','clients_followup_log.*');
    if($request->group){
    ...
    $query = $data->where('clients_followup_log.next_followup_date', '!=', ' ')
              ->where('clients_followup_log.next_followup_date', '>=', now())->whereIn('clients_followup_log.created_by',$emp_managers)->paginate(10);
              
    foreach( $query as $row){
        $followups_date = DB::connection('sales_db')->table('clients_followup_log')->where('id',$row->id)->where('client_id',$row->client_id)->first();
        
        $no_of_times_carry_forward = DB::connection('sales_db')->table('clients_followup_log')->where('client_id',$row->client_id)->where('next_followup_date','!=','')->count();
        ...
        $product = DB::connection('sales_db')->table('product')->where('id',$row->product_id)->first();
         ...
         $data_array[] = array(
           'id'=>$row->id,
           'previous_followup_date'=>$followups_date->created_date,
           'next_followup_date'=>$followups_date->next_followup_date,
           'no_of_times_carray_forwad'=>$no_of_times_carry_forward,
...
public function get_active_clients_details(Request $request) {
    ...
    $data = DB::connection('sales_db')->table('client_info')->where('status', 2);
    if ($request->group) {
        $data->where('group_id', $request->group);
    }
...
public function get_active_packages_list(Request $request) {
    ...
    foreach ($query as $row) {
        ...
        $client = DB::connection('sales_db')->table('client_info')->where('client_id', $row->client_id)->first();
