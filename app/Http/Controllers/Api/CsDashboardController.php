...
class CsDashboardController extends Controller
{
    public function cs_dashboard_show_count(Request $request)
    {
        ...
        $per_day_verified_time = DB::connection('sales_db')
            ->table('enquiry_info')
            ->where('cs_verified', 1)
            ->whereRaw('TIME(created_date) BETWEEN ? AND ?', [$startTime, $endTime]);
        ...
        if ($request->product) {
            ...
            $per_day_verified_time->where('product_id', $productId);
       }
        ...
        if($request->group) {
            ...
            $per_day_verified_time->where('group_id',$groupId);
        ...
        if ($request->service) {
            ...
            $per_day_verified_time->where('service_id',$serviceId);
        }
        ...
        if($request->category){
                ...
                $per_day_verified_time->where('category_id',$categoryId);
        ...
        if($request->emp){
             ...
             $per_day_verified_time->where('verified_by',$request->emp);
        ...
        if ($request->start_date && $request->end_date) {
            ...
            $query->whereDate('verifed_date', '>=', $startDate)
                  ->whereDate('verifed_date', '<=', $endDate);
            ...
            ->whereDate('start_stamp', '<=', $endDate);
            $per_day_verified_time->whereDate('verifed_date', '>=', $startDate)
            ->whereDate('verifed_date', '<=', $endDate);
            ...
            $query->whereMonth('verifed_date', $currentmonth)
                  ->whereYear('verifed_date',$currentyear);
            ...
            $per_day_verified_time->whereDate('verifed_date',Carbon::now());
        ...
        $data = $query->get();
        $per_day_data = $per_day_verified_time->get();
        foreach ($data as $row) {
            if ($row->verifed_date) {
        ...
        foreach ($per_day_data as $time) {
            if ($time->verifed_date) {
                $createdAt = $time->created_date;
                $verifiedAt = $time->verifed_date;
                $diffInMinutes = Carbon::parse($createdAt)->diffInMinutes(Carbon::parse($verifiedAt));
                $per_day_count++;
                $per_day_time += $diffInMinutes;
            }
        }
    
        $total_average_time_per_day = $per_day_count > 0 
            ? round($per_day_time/$count) 
            : 0;
    
        $data = [
            'verified_time_average' => round($total_average_time) . ' Minutes',
            ...
            'per_day_time'=>round($total_average_time_per_day).' '.'Minutes',
        ];
    ...
    public function get_category_based_on_service_id($id){
        $data = DB::connection('sales_db')->table('product_category')
       ...
       $emp_details = DB::table('employee_managers')
        ...
        foreach($query as $row){
            ...
            $customer = DB::connection('sales_db')->table('customer_info')->where('id',$row->customer_id)->first();
                ...
                $data_array[] = array('id'=>$row->id,'product'=>$product->product_name??'','group'=>$group->name??'','service'=>$service->service_name??'','category'=> $category->category_name??'','customer'=>$customer->name??'','created_date'=>$row->created_date??'','verified_date'=>$row->verifed_date??'','emp_fname'=> $verified_by->emp_fname??'','emp_lname'=>$verified_by->emp_lame??'','time_gap'=>$diffInMinutes??'');
    ...
    public function lead_verification_tat(Request $request)
{
    $per_day_count = 0;
    $per_day_time = 0;

    // Get employees reporting to the given employee ID
    $emp_details = DB::table('employee_managers')
        ->where(function($query) use ($request) {
        ...
        ->pluck('emp_id')
        ->toArray();
    $emp_details[] = $request->emp_id;
    $emp_details = array_unique($emp_details);

    $data = DB::connection('sales_db')->table('enquiry_info')
        ->whereDate('verifed_date', Carbon::now()->format('Y-m-d'))
        ->whereIn('verified_by', $emp_details)
        ->get();
    ...
    $total_average_time_per_day = $per_day_count > 0
        ? round($per_day_time / $per_day_count)
        : 0;

    if ($total_average_time_per_day > 4) {
        $message = 'Lead Verification Time More Than 4 Minutes';
        $type = 'lead verification Time';
        $data_array = [
...
public function lead_verification_status_seen_status($emp_id){
    DB::table('emp_notifications')
            ->where('type', 'lead verification Time')
            ->where('emp_id', $emp_id)
