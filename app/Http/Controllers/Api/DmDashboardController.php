      ...
      public function __construct()
      {
       ...
       public function dm_dashboard_count(Request $request)
       {
        $count = 0;
           ...
           $total_valid_enquiry = $total_enquiry->whereNotIn('followup_status',[18,19,20,21,6,10])->count();
...
public function lead_not_sent_due_to_quality(Request $request){
    $data_array = [];
    $data = DB::connection('sales_db')->table('enquiry_info')->whereIn('followup_status',[18,19,20,21,6,10]);
    if($request->group){
        $data->where('group_id',$request->group);
