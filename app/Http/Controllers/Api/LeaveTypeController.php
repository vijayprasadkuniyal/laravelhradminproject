...
class LeaveTypeController extends Controller
{
     ...
     // return $request->all();
      ...
      $leave->is_confirmation_leave = $request->confirm_leave;
...
public function get_active_leavetype($id){
  //return 'jj';
    ...
    ->where('emp_leave_blance.is_confirmation_leave', 1)
...
public function leave_permission($id){
   $leave_blance = LeaveType::where('assign_when_request',1)->where('status',1)->get();
  ...
  public function update_leave_permission(Request $request){
    ...
    $data_array = array('leave_id'=>$leave->id,
            'emp_id'=>$request->emp_id,'year'=>$currentYear,'current_year_assign_leave'=>$leave->leave_days,'total_leave'=>$leave->leave_days,'remaining'=>$leave->leave_days,'is_probation_leave'=>$leave->is_probation_leave,
              'is_request_leave'=>$leave->assign_when_request,'is_confirmation_leave'=>$leave->is_confirmation_leave);
...
public function assign_leave(){
  ...
  foreach($data as $row){
    if($row->is_confirmed==0){
      $probation_leave = LeaveType::where('is_probation_leave',1)->where('assign_when_request',0)->where('status',1)->get();
        ...
        if(!$check_exists){
          $data_array = array('leave_id'=>$probation->id,
            'emp_id'=>$row->emp_id,'year'=>$currentYear,'current_year_assign_leave'=>$assigned_leave,'total_leave'=>$assigned_leave,'remaining'=>$assigned_leave,'is_probation_leave'=>$probation->is_probation_leave,
              'is_request_leave'=>$probation->assign_when_request,'is_confirmation_leave'=>$probation->is_confirmation_leave);
              EmployeeLeaveBlance::insert($data_array);
           }
        }
      ...
      $confirmed_leave = LeaveType::where('is_confirmation_leave',1)->where('assign_when_request',0)->where('status',1)->get();
      foreach($confirmed_leave as $confirm){
        ...
        if(!$check_exists){
          $data_array = array('leave_id'=>$confirm->id,
            'emp_id'=>$row->emp_id,'year'=>$currentYear,'current_year_assign_leave'=>$assigned_leave,'total_leave'=>$assigned_leave+$pl,'remaining'=>$assigned_leave+$pl,'is_probation_leave'=>$confirm->is_probation_leave,'is_confirmation_leave'=>$confirm->is_confirmation_leave,
  ...
  public function save_emp_leave(Request $request)
{
    ...
    $leave_details = EmployeeLeaveBlance::where('emp_id',$request->emp_id)->where('leave_id',$request->leave_type)
    ->where('year',Carbon::now()->year)->first();

    if($leave_details->remaining>0 && $leave_details->remaining>=$request->no_of_days ){
      $remaining = $leave_details->remaining - $request->no_of_days;
    }
    else{
       $remaining = $leave_details->remaining;
    ...
    if (!$check_leave_blance || $check_leave_blance->remaining < $request->no_of_days) {
    ...
    if ($total_assign_leave_in_a_month && $total_assign_leave_in_a_month->at_a_time < $request->no_of_days) {
        return response()->json(['message' => 'You can not Apply More Than'.$total_assign_leave_in_a_month->at_a_time]);
    }
        $leave->save();

        EmployeeLeaveBlance::where('emp_id',$request->emp_id)->where('leave_id',$request->leave_type)
        ->where('year',Carbon::now()->year)->update(['leave_taken'=>$leave_details->leave_taken + $request->no_of_days,
        'remaining'=>$remaining]);

        $emp_name = BasicInfo::where('emp_id',$request->emp_id)->first();
        $message = $emp_name->emp_fname.' '.'Send Leave Request Please Click TO View';
        
        $notification_data = array('emp_id'=>$emp_name->reporting_manager,'notification'=>$message,'type'=>'leave','action_by'=>$emp_name->emp_fname);
         DB::table('emp_notifications')->insert($notification_data);
          $type = 'leave';
          event(new LeaveEvent($message,$emp_name->reporting_manager,$type));
          return response()->json(['status'=>200,'message' => 'Leave saved successfully.']);

    
}

public function show_leave_type_based_on_emp_status($id){
    $data = BasicInfo::where('emp_id',$id)->first();
    if($data->is_confirmed == 0){
        $leave_details = LeaveType::where('is_probation_leave',1)->where('assign_when_request',1)->get(['id','leave_name']);

    }
    elseif($data->is_confirmed ==1){
        $leave_details = LeaveType::where('is_confirmation_leave',1)->where('assign_when_request',1)->get(['id','leave_name']);

    }

    return response()->json(['status'=>200,'data'=>$leave_details]);
}

public function save_leave_assign_request(Request $request){
    $data = array('emp_id'=>$request->emp_id,'leave_id'=>$request->leave_type,'remark'=>$request->remark);
    DB::table('leave_request_details')->insert($data);
    return response()->json(['status'=>200,'message'=>'Request Send Successfully']);
}

public function get_leave_request_details($id)
{
    $data = DB::table('leave_request_details as lr')
        ->leftJoin('emp_basic_info as e1', 'lr.emp_id', '=', 'e1.emp_id')
        ->leftJoin('create_leave_type as clt', 'clt.id', '=', 'lr.leave_id')
        ->leftJoin('emp_basic_info as e2', 'lr.action_by', '=', 'e2.emp_id')
        ->select(
            DB::raw('COALESCE(e2.emp_fname, "") as approved_by_fname'),
            DB::raw('COALESCE(e2.emp_lame, "") as approved_by_lame'),
            DB::raw('COALESCE(e1.emp_fname, "") as emp_fname'),
            DB::raw('COALESCE(e1.emp_lame, "") as emp_lame'),
            DB::raw('COALESCE(clt.leave_name, "") as leave_name'),
            DB::raw("
                CASE 
                    WHEN lr.status = 1 THEN 'Approved'
                    WHEN lr.status = 0 THEN 'Pending'
                    WHEN lr.status = 2 THEN 'Rejected'
                    ELSE 'Unknown'
                END as status_text
            "),
            'lr.remark',
...
public function leave_request_details_hr_dashboard(Request $request){
    $data = DB::table('leave_request_details as lr')
    ->leftJoin('emp_basic_info as e1', 'lr.emp_id', '=', 'e1.emp_id')
    ->leftJoin('create_leave_type as clt', 'clt.id', '=', 'lr.leave_id')
    ->leftJoin('emp_basic_info as e2', 'lr.action_by', '=', 'e2.emp_id')
    ->leftjoin('department','department.id','e1.dept_id')
    ->select(
        DB::raw('COALESCE(e2.emp_fname, "") as approved_by_fname'),
        DB::raw('COALESCE(e2.emp_lame, "") as approved_by_lame'),
        DB::raw('COALESCE(e1.emp_fname, "") as emp_fname'),
...
public function action_on_emp_leave_request(Request $request){
    DB::table('leave_request_details')->where('emp_id',$request->emp_id)
    ->where('id',$request->id)->update(['status'=>$request->status,'action_by'=>$request->action_by]);
