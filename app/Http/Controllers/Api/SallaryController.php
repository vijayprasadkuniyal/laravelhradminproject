...
public function deduction_attribute(){
    $deduction = Salaryattribute::where('is_deduction_attribute', 1)->where('is_contribution_attribute', 0)->get();
    $attribute = [];
...
public function save_salary(Request $request)
   {
    ...
    ->whereYear('attendance.attendance_date', Carbon::now()->year)->get();

$roster = DB::table('emp_roster')
    ->whereMonth('week_off',Carbon::now()->month)
...
foreach ($data as $item) {
 ...
 if (($weekday == 'Saturday' || $weekday == 'Sunday') && $item->dept_id != 4) {
     $previousFriday = $date->copy()->subDays($weekday === 'Saturday' ? 1 : 2);
     $nextMonday = $date->copy()->addDays($weekday === 'Saturday' ? 2 : 1);

     $checkFridayLeave = DB::table('leave_details')->where('date', $previousFriday->format('Y-m-d'))->first();
     $checkMondayLeave = DB::table('leave_details')->where('date', $nextMonday->format('Y-m-d'))->first();
     $fridayAttendance = DB::table('attendance')->where('emp_id', $empId)->where('attendance_date', $previousFriday->format('Y-m-d'))->first();
     $mondayAttendance = DB::table('attendance')->where('emp_id', $empId)->where('attendance_date', $nextMonday->format('Y-m-d'))->first();

     if (($checkFridayLeave && $checkMondayLeave) ||
         ($fridayAttendance && $fridayAttendance->leave_status == 1 && $mondayAttendance && $mondayAttendance->leave_status == 1) ||
         ($fridayAttendance && $fridayAttendance->status != 0 && $mondayAttendance && $mondayAttendance->status != 0) ||
         ($checkFridayLeave && $mondayAttendance && $mondayAttendance->status != 0) ||
         ($checkMondayLeave && $fridayAttendance && $fridayAttendance->status != 0)) {
         $working_days += 1;
     } elseif (($fridayAttendance && $fridayAttendance->status != 0) || ($mondayAttendance && $mondayAttendance->status != 0)) {
         $working_days += 1;
     } else {
         $status = 'A';
     }
 } else {
     $isRosterOff = $item->dept_id == 4 && isset($roster[$empId]) && $roster[$empId]->where('week_off', $formattedDate)->isNotEmpty();
     ...
     if ($isHoliday) {
         ...
         $working_days += 1;
     } elseif (($item->status == 2 && $item->leave_status == 0 && $item->is_half_day == 0)
         || ($item->status == 2 && $item->leave_status == 0 && $item->is_half_day == 1)) {
         $working_days += 0.5;
     } elseif ($item->is_half_day == 1 && $item->leave_status == 1 && $item->status == 0) {
         $working_days += 0.5;
     } elseif ($item->is_half_day == 1 && $item->leave_status == 1 && $item->status != 0) {
         $working_days += 1;
 ...
 $groupedData[$empid]['total_working_days'] += $working_days;
}
$data_array = [];
$now = Carbon::now();
$year =  $now->year;
...
foreach ($groupedData as $empId => $empData) {
$gross_salary = 0;

$emp_salary = DB::table('emp_salary_info')->leftjoin('salary_attribute','salary_attribute.id','emp_salary_info.attribute_id')

 ->select('salary_attribute.id','salary_attribute.deduction_from','salary_attribute.percentage','emp_salary_info.*')

 ->where('emp_id',$empData['emp_id'])->get();

 //return $emp_salary;

 $basic_salary_row = SalLary::where('attribute_id',1)->where('emp_id',$empData['emp_id'])->first();
 if($basic_salary_row){
    $new_basic_salary = round($basic_salary_row->amount_per_month/$daysInMonth);
    $basic_salary = $new_basic_salary*$empData['total_working_days'];
...
if($salary->is_deduction==0 && $salary->is_contribute==0){
    if($empData['total_working_days']>0){
      $per_day_salary = round($salary->amount_per_month/$daysInMonth);

      $amount = $empData['total_working_days']*$per_day_salary;
   ...
   $data_array[] = array('emp_id'=>$salary->emp_id,'actual_amount'=>$salary->amount_per_month,'amount'=>$amount,'is_deduction'=>$salary->is_deduction,'is_contribute'=>$salary->is_contribute,'month'=>$month,'year'=>$year,'attribute_id'=>$salary->attribute_id,'working_days'=>$empData['total_working_days']);
 }
 else{
   //return 'kkk';
   if($salary->deduction_from =='basic' && $salary->percentage!=''){
     //return $salary->percentage;
     $amount = round(($basic_salary*$salary->percentage)/100);

   }

   elseif($salary->deduction_from =='gross' && $salary->percentage!=''){

      $amount = round(($gross_salary*$salary->percentage)/100);
    }
    else{
     $amount = $salary->amount_per_month;
...
// DB::table('emp_monthly_salary')->insert($data_array);
