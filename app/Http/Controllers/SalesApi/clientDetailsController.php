...
class clientDetailsController extends Controller
{
    ...
    {
        $compInfo = DB::connection('sales_db')->table('company_info')
            ->select('client_name', 'business_name', 'status', 'exe_id')
            ->where('mobile_no', $mobileNo)
            ->get();

        $exeIds = $compInfo->pluck('exe_id')->toArray();
        $empNames = DB::table('emp_basic_info')
            ->whereIn('emp_id', $exeIds)
            ->selectRaw("emp_id, GROUP_CONCAT(emp_fname, ' ', emp_lame) as emp_name")
            ->get()
            ->keyBy('emp_id');

        $compInfo->map(function ($item) use ($empNames) {
            $item->emp_name = $empNames->get($item->exe_id)->emp_name ?? null;
            return $item;
        });

        if (!$compInfo->isEmpty()) {
            return response()->json(['status' => 200, 'message' => 'Client already added as a company.', 'data' => $compInfo]);
        }

        $clientInfo = DB::connection('sales_db')->table('client_info')
            ->select('client_name', 'status', 'exe_id')
            ->where('mobile_no', $mobileNo)
            ->get();

        $exeIds = $clientInfo->pluck('exe_id')->toArray();
        $empNames = DB::table('emp_basic_info')
            ->whereIn('emp_id', $exeIds)
            ->selectRaw("emp_id, GROUP_CONCAT(emp_fname, ' ', emp_lame) as emp_name")
            ->get()
        ...
        $clientInfo->map(function ($item) use ($empNames) {
            $item->emp_name = $empNames->get($item->exe_id)->emp_name ?? null;
            return $item;
        });

        if (!$clientInfo->isEmpty()) {
            return response()->json(['status' => 200, 'message' => 'Client already added as a client.', 'data' => $clientInfo]);
        }

        $guestClientInfo = DB::connection('sales_db')->table('guest_user_details')
            ->select('name as client_name', 'business_name', 'status', 'created_by')
            ->where('mobile_no', $mobileNo)
            ->get();

        $createdByIds = $guestClientInfo->pluck('created_by')->toArray();
        $empNames = DB::table('emp_basic_info')
        ...
        $guestClientInfo->map(function ($item) use ($empNames) {
            $item->emp_name = $empNames->get($item->created_by)->emp_name ?? null;
            return $item;
        });

        if (!$guestClientInfo->isEmpty()) {
            return response()->json(['status' => 200, 'message' => 'Client already added as a guest client.', 'data' => $guestClientInfo]);
        }

        return response()->json(['status' => 201, 'message' => 'This is a new client']);
    }

    public function check_new_mobile_number(Request $request)
    {
        ...
        if(!$compInfo->isEmpty())
        {
            return response()->json(['status' => 200, 'message' => 'Mobile number is already registered with another company.','data' => $compInfo]);
        }
        ...
        if(!$clientInfo->isEmpty())
        {
            return response()->json(['status' => 200, 'message' => 'Mobile number is already registered with another client.', 'data' => $clientInfo]);
        }
        return response()->json(['status' => 201, 'message' => 'This is a new client']);
    }
    ...
    public function add_client_followup(Request $request)
    {
        ...
        if($followupId)
        {
            ...
            {
                $compInfo = DB::connection('sales_db')->table('company_info as ci')
                ->select('status')->where('comp_id','=',$request->company_id)
                ->first();
                if($compInfo->status == '4')
                {   
                    $updateInfo = [
                    ...
                    $assignClient = DB::connection('sales_db')
                        ->table('company_info')
                    ...
                    if ($assignClient) {
                        return response()->json([
                            'status' => 200,
                            'message' => 'Follow-up created successfully and client auto-assigned to executive.',
                            'data' => $followupId,
                        ]);
                    } else {
                        return response()->json([
                            'status' => 200,
                            'message' => 'Follow-up created successfully, but client could not be assigned.',
                            'data' => $followupId,
                        ]);
                    }
                }
    ...
    public function check_last_followup_details(Request $request)
    {
        ...
        if($followupInfo)
        {
            if(date('Y-m-d') > $nextFollowupDate) 
            {
                $remaingDays='0';
                $followupStatus = '1'; // Show follow-up button
    ...
    public function add_new_company_details(Request $request)
    {
        ...
        $validator = Validator::make($input,[
            ...
            $clientInfo = DB::connection('sales_db')->table('client_info')->select('client_name')->where('client_id',$request->client_id)->first();
            ...
            if($companyId)
            {
                ...
                if($docUpdate)
                {
                    return response()->json(['status'=>200,'message'=>'Company Created successfully','data'=>$companyId]);
                }
                else
                {
                    return response()->json(['status'=>200,'message'=>'Company Created successfully','data'=>$companyId]);
    ...
    //     ->select('product.product_name','package_duration.name as duration_name','package_info.package_amount','package_info.tax_amount','package_info.package_start_date','package_info.package_end_date','package_info.package_id','package_info.total_lead as committed_lead','package_info.sent_lead','package_info.created_by','package_info.created_date','package_info.admin_status','package_info.finance_status','package_info.package_status','package_info.paid_amount','package_info.due_amount','package_info.due_status','package_info.due_lead','package_info.package_name',

    //     'pre_package.id','pre_package.package_status as status','pre_package.total_lead','pre_package.is_partial_payment','pre_package.total_amount as package_price','package_category.name as category_name','package_type.name as package_type_name','pre_package.city_id','pre_package.group_id','company_info.business_name','company_info.mobile_no')
    ...
    {
        $clientPackageInfo = DB::connection('sales_db')->table('package_info')
        ->select('product.product_name','package_duration.name as duration_name','package_info.package_amount','package_info.tax_amount','package_info.package_start_date','package_info.package_end_date','package_info.package_id','package_info.total_lead as committed_lead','package_info.sent_lead','package_info.created_by','package_info.created_date','package_info.admin_status','package_info.finance_status','package_info.package_status','package_info.paid_amount','package_info.due_amount','package_info.due_status','package_info.due_lead','package_info.package_name','package_info.group_id as package_group_id','package_info.city_id as package_city_id',

        'pre_package.id','pre_package.package_status as status','pre_package.total_lead','pre_package.is_partial_payment','pre_package.total_amount as package_price','package_category.name as category_name','package_type.name as package_type_name','pre_package.city_id','pre_package.group_id','company_info.business_name','company_info.mobile_no','company_info.exe_id','group_names.name as groupname','package_info.package_mobile')
        ...
        ->leftJoin('group_names', function ($join) {
            $join->whereRaw("FIND_IN_SET(group_names.group_id, package_info.group_id)");
        })
        ->leftJoin('payment_history',function($join1)
        {
            $join1->whereRaw("Find_in_set(payment_history.package_id,package_info.package_id)");
        ...
        $clientPackageInfo->map(function ($item) {
            $empName = DB::table('emp_basic_info')
                ->selectRaw("GROUP_CONCAT(emp_fname, ' ', emp_lame) as emp_name")
    ...
    {
        $walletHistory = DB::connection('sales_db')->table('wallet_history')
        ->select('client_info.client_name','wallet_history.*','bank_info.bank_name as bankName')
    ...
    public function check_company_pancard_number($pancard_no)
    {
        $check_pancard = DB::connection('sales_db')->table('document_info')
        ->select('doc_number')
        ->where(array('doc_number'=>$pancard_no,'doc_type_id'=>1))
        ->first();
    ...
    //     ->select('company_info.comp_id','client_info.client_name','company_info.business_name','company_info.status','company_info.created_at','company_info.exe_id','company_info.client_id','company_info.created_by')
    //     ->leftJoin('company_info','company_info.comp_id','=','document_info.comp_id')
    //     ->leftJoin('client_info','client_info.client_id','=','document_info.client_id')
    //     ->where(array('document_info.status'=>0))
    //     ->where('document_info.client_id','!=','')
    ...
    //     ->leftJoin('group_names as gm', 'gm.group_id','=','ci.group_id')
    //     ->where(function ($query) {
    //         $query->where('pi.admin_status', 0)
    //             ->where('pi.package_status', 0)
    //             ->where('pi.finance_status', 1)
    ...
    //     ->leftJoin('company_info as ci', 'ci.comp_id', '=', 'pi.comp_id')
    //     ->leftJoin('package_duration as pd', 'pd.id', '=', 'pi.package_duration')
    //     ->leftJoin('group_names as gm', 'gm.group_id', '=', 'pi.group_id')
    ...
    public function update_package_area(Request $request)
    {
        ...
        if($insertData)
        {
            ...
            //remove 0 to group string
            $groupIds = explode(',', $request->group_id);
            $filteredArray = array_filter($groupIds, function($groups)
            {
                return $groups !== '0';
            });
            $groupIds = implode(',', $filteredArray);

            //remove 0 to city string
            $cityIds = explode(',', $request->city_id);
            $cityFilteredArray = array_filter($cityIds, function($city)
            {
                return $city !== '0';
            });
            $cityIds = implode(',', $cityFilteredArray);

            //remove 0 to locality  string
            $localityIds = explode(',', $request->locality_id);
            $localityFilteredArray = array_filter($localityIds, function($localityId)
            {
                return $localityId !== '0';
            });
            $localityIds = implode(',', $localityFilteredArray);
    ...
    public function update_package_to_location_area(Request $request)
    {
        ...
        if ($insertData)
        {
            $to_city_array = explode(',', $request->to_city);
            $getLocality = DB::connection('sales_db')
                ->table('rsms_city')
                ->selectRaw('GROUP_CONCAT(city_id) as localities')
                ->whereIn('locality_id', $to_city_array)
                ->first();
                if(isset($getLocality->localities) && !empty($getLocality->localities))
                {
                    //remove 0 to group string
                    $groupIds = explode(',', $request->to_group);
                    $filteredArray = array_filter($groupIds, function($groups) {
                        return $groups !== '0';
                    });
                    $groupIds = implode(',', $filteredArray);
                    ...
                    //remove 0 to city string
                    $cityIds = explode(',', $request->to_city);
                    $cityFilteredArray = array_filter($cityIds, function($city) {
                        return $city !== '0';
                    });
                    $cityIds = implode(',', $cityFilteredArray);
                    ...
                    $areaUpdate = [
                        'to_group'=> $groupIds,
                        'to_city'=> $cityIds,
                        'to_locality'=> $getLocality->localities,
                    ];

                    $packagesInfo = DB::connection('sales_db')
                        ->table('package_info')
    ...
    //     ->select('client_info.id','client_info.client_id','client_info.client_name','client_info.adhar_card','client_info.email','client_info.mobile_no','client_info.city','client_info.exe_id','company_info.status','company_info.business_name','company_info.mobile_no as comp_mobile_no','company_info.email as comp_email','company_info.exe_id as comp_exe_id')
    //     ->leftJoin('client_info','client_info.client_id','=','company_info.client_id')
    //     ->where('company_info.status',2)
    //     ->whereIn('company_info.exe_id',$empIds)
    //     ->when($mobileNumber, fn ($query) => $query->where('company_info.mobile_no','LIKE','%'.$mobileNumber.'%'))
    //     ->when($companyName, fn ($query) => $query->where('company_info.business_name','LIKE','%'.$companyName.'%'))
    //     ->paginate($request->per_page);

    //     $clientInfo->map(function($item){
    //         $empName = DB::table('emp_basic_info')
    //         ->selectRaw("GROUP_CONCAT(emp_fname ,' ', emp_lame) as emp_name")
    //         ->where('emp_id',$item->comp_exe_id)
    //         ->first();
    //         $item->emp_name = $empName->emp_name;
    //         return $item;    
    //     });
    ...
    public function get_active_client_details(Request $request)
    {
        ...
        $clientInfo = DB::connection('sales_db')->table('company_info')
        ->select('client_info.id','client_info.client_id','client_info.client_name','client_info.adhar_card','client_info.email','client_info.mobile_no','company_info.city','client_info.exe_id','company_info.status','company_info.business_name','company_info.mobile_no as comp_mobile_no','company_info.email as comp_email','company_info.exe_id as comp_exe_id','group_names.name as group_name','cities.city_name')
        ->leftJoin('client_info','client_info.client_id','=','company_info.client_id')
        ->leftJoin('group_names','group_names.group_id','=','company_info.group_id')
        ->leftJoin('cities','cities.id','=','company_info.city')

        ->where('company_info.status',2)
        ->whereIn('company_info.exe_id',$empIds)
        ->when($mobileNumber, fn ($query) => $query->where('company_info.mobile_no','LIKE','%'.$mobileNumber.'%'))
        ->when($companyName, fn ($query) => $query->where('company_info.business_name','LIKE','%'.$companyName.'%'))
        ->when($selectedGroup, fn ($query) => $query->where('company_info.group_id',$selectedGroup))
        ->paginate($request->per_page);
    ...
    //     ->leftJoin('group_names','group_names.group_id','=','company_info.group_id')
    //     ->leftJoin('cities','cities.id','=','company_info.city')
    //     ->where('company_info.status',4)
    //     //->whereIn('company_info.exe_id',$empIds)
    //     ->when(is_array($empIds) && !empty($empIds), fn($query) => $query->whereIn('company_info.exe_id', $empIds))

    //     //->when($empIds, fn ($query) => $query->whereIn('company_info.exe_id','=',$empIds))
    //     ->when($mobileNumber, fn ($query) => $query->where('company_info.mobile_no','LIKE','%'.$mobileNumber.'%'))
    //     ->when($companyName, fn ($query) => $query->where('company_info.business_name','LIKE','%'.$companyName.'%'))
    //     ->when($selectedGroup, fn ($query) => $query->where('company_info.group_id',$selectedGroup))
    //     ->paginate($request->per_page);
    ...
    public function get_expire_client_details(Request $request)
    {
        ...
        $clientInfo = DB::connection('sales_db')->table('company_info')
        ->select('client_info.id','client_info.client_id','client_info.client_name','client_info.adhar_card','client_info.email','client_info.mobile_no','company_info.city','client_info.exe_id','company_info.status','company_info.business_name','company_info.mobile_no as comp_mobile_no','company_info.email as comp_email','company_info.exe_id as comp_exe_id','group_names.name as group_name','cities.city_name','client_info.status as client_status')
        ->leftJoin('client_info','client_info.client_id','=','company_info.client_id')
        ->leftJoin('group_names','group_names.group_id','=','company_info.group_id')
        ->leftJoin('cities','cities.id','=','company_info.city')
        // ->leftJoin('clients_followup_log as cfl', function($join)
        // {
        //     $join->on('cfl.comp_id', '=', 'company_info.comp_id');
        // })
        ...
        // ->orWhereDate('cfl.next_followup_date', '<', $today)
        ->paginate($request->per_page);
...
//     // Get employee IDs based on emp_id, or set empty array for filtering
//     $empIds = $request->emp_id ? self::get_executive_id($request->emp_id) : [];
    
//     // Collect input filters with conditions
//     $mobileNumber = strlen($request->mobileNo ?? '') > 2 ? $request->mobileNo : '';
//     $companyName = strlen($request->companyName ?? '') > 2 ? $request->companyName : '';
//     $selectedGroup = $request->selectedGroup ?? '';

//     // Main query with conditional joins and filters
//     $clientInfo = DB::connection('sales_db')->table('company_info')
//         ->select(
//             'client_info.id',
//             'client_info.client_id',
//             'client_info.client_name',
//             'client_info.adhar_card',
//             'client_info.email',
//             'client_info.mobile_no',
//             'company_info.city',
//             'client_info.exe_id',
//             'company_info.status',
//             'company_info.business_name',
//             'company_info.mobile_no as comp_mobile_no',
//             'company_info.email as comp_email',
//             'company_info.exe_id as comp_exe_id',
//             'group_names.name as group_name',
//             'cities.city_name',
//             'client_info.status as client_status'
//            // DB::raw("GROUP_CONCAT(emp_basic_info.emp_fname, ' ', emp_basic_info.emp_lame) as emp_name")
//         )
//         ->leftJoin('client_info', 'client_info.client_id', '=', 'company_info.client_id')
//         ->leftJoin('group_names', 'group_names.group_id', '=', 'company_info.group_id')
//         ->leftJoin('cities', 'cities.id', '=', 'company_info.city')
//         ->leftJoin('clients_followup_log as cfl', function ($join) {
//             $join->on('cfl.comp_id', '=', 'company_info.comp_id')
//                  ->whereDate('cfl.next_followup_date', '<', date('Y-m-d'))
//                  ->where('cfl.client_type', '2');
//         })
//        // ->leftJoin('emp_basic_info', 'emp_basic_info.emp_id', '=', 'company_info.exe_id') // Joining emp_basic_info table for emp_name
//         ->when($empIds, fn ($query) => $query->whereIn('company_info.exe_id', $empIds))
//         ->when($mobileNumber, fn ($query) => $query->where('company_info.mobile_no', 'LIKE', '%' . $mobileNumber . '%'))
//         ->when($companyName, fn ($query) => $query->where('company_info.business_name', 'LIKE', '%' . $companyName . '%'))
//         ->when($selectedGroup, fn ($query) => $query->where('company_info.group_id', $selectedGroup))
//         ->where('company_info.status', 4)
//         ->groupBy('client_info.id') // Grouping for emp_name aggregation
//         ->paginate($request->per_page);
...
// }
    ...
    //     ->where(function ($query) use ($empIds)
    //     {
    //         $query->whereIn('guest_user_details.created_by', $empIds)
    //               ->orWhere('guest_user_details.created_by', 'RIMS');
    //     })
        
        
    //     ->when($mobileNumber, fn ($query) => $query->where('guest_user_details.mobile_no','LIKE','%'.$mobileNumber.'%'))
    //     ->when($companyName, fn ($query) => $query->where('guest_user_details.business_name','LIKE','%'.$companyName.'%'))
        
    //     ->where(array('guest_user_details.is_matured'=>0))
    //     //->where('guest_user_details.is_verification',0)
    ...
    public function get_client_details(Request $request)
    {
        ...
        $clientInfo = DB::connection('sales_db')->table('guest_user_details')
            ->select(
                'guest_user_details.id',
                'guest_user_details.name',
                'guest_user_details.mobile_no',
                'guest_user_details.email_id',
                'guest_user_details.business_name',
                'product.product_name',
                'product_service.service_name',
                'product_category.category_name',
                'group_names.name as group_name',
                'cfl.next_followup_date'
            )
            ->leftJoin('group_names', 'group_names.group_id', '=', 'guest_user_details.city')
            ->leftJoin('product', 'product.id', '=', 'guest_user_details.product_id')
            ->leftJoin('product_service', 'product_service.id', '=', 'guest_user_details.service_id')
            ->leftJoin('product_category', 'product_category.id', '=', 'guest_user_details.category')
            ->leftJoin('clients_followup_log as cfl', function($join) {
                $join->on('cfl.client_id', '=', 'guest_user_details.id');
            })

            ->when($empIds, function ($query, $empIds)
            {
                return $query->whereIn('guest_user_details.created_by', $empIds);
            })

            ->when($mobileNumber, function ($query, $mobileNumber) {
                return $query->where('guest_user_details.mobile_no', 'LIKE', '%' . $mobileNumber . '%');
            })
            ->when($companyName, function ($query, $companyName) {
                return $query->where('guest_user_details.business_name', 'LIKE', '%' . $companyName . '%');
            })
            ->where('guest_user_details.is_matured', 0)
            ->where('guest_user_details.is_verification', 0)
            //->where('guest_user_details.status', 0)
            ->where('cfl.client_type','1')
            ->orWhereDate('cfl.next_followup_date', '<', $today)
            ->orderBy('guest_user_details.id', 'DESC')
            ->paginate($request->per_page);
    ...
    public function get_pre_package_by_category(Request $request)
    {
        ...
        $prePackages = DB::connection('sales_db')->table('pre_package')
            ...
            ->orderBy('pre_package.service_id', 'ASC')
            ->get();
    ...
    public function get_company_list(Request $request)
    {
        ...
        ->table('company_info')
        ->select('comp_id','business_name','mobile_no')
        ->where('client_id',$clientId)
        ->get();
        return response()->json(['status' => 200, 'message' => 'Company List','data'=>$companyList]);
    }

    // public function select_company_detail(Request $request)
    // {
    //     $clientId=$request->client_id;
    //     $compId= $request->comp_id;
    //     $emp_id=$request->emp_id;
    //     $cond = array('client_id'=>$clientId ,'comp_id'=>$compId);
    //     $companyList = DB::connection('sales_db')
    //     ->table('company_info')
    //     ->where($cond)
    //     // ->where('comp_ids', $compId)
    //     // /->where('exe_id' , $compId)
    //     ->first();
    //     // print_r($companyList->status);
    //     if(!Empty($companyList->status)){
    //         if($companyList->status  == '2' || $companyList->status == '3') {
    //             $companyList = DB::connection('sales_db')
    //             ->table('company_info')
    ...
    //             ->where('exe_id' , $emp_id)
    //             ->first(); 
    //             return response()->json(['status' => 200, 'message' => 'Company detail','data'=>$companyList]);
    //         } else  if($companyList->status  == '4') {
    //             $companyList = DB::connection('sales_db')
    //             ->table('company_info')
    //             ->select('email','mobile_no')
    //             ->where('client_id',$clientId)
    ...
    //             return response()->json(['status' => 200, 'message' => 'Company detail','data'=>$companyList]);
    //         } else {
    //             return response()->json(['status' => 400, 'message' => 'Contact to admin']);
    //         }
    //     }else{
    //         return response()->json(['status' => 400, 'message' => 'Contact to admin']);
    //     }
    ...
    public function update_request_status(Request $request)
    {
        ...
        public function update_uplode_company_document(Request $request)
    {
        $input = $request->all();
        $validator = Validator::make($input, [
          'doc_id' => 'required',
    ...
    public function uplode_new_company_document(Request $request)
    {
        $input = $request->all();
        $validator = Validator::make($input, [
    ...
    public function get_document_listing()
    {
        $documentDetails = DB::connection('sales_db')->table('document_type')
        ->where('status', 1)
        ->get();
        return response()->json(['status' => 200, 'message' => 'Document Listing', 'data' => $documentDetails]);
    }
    ...
    public function check_duplicate_transaction_edit(Request $request)
    {
        $trnInfo = DB::connection('sales_db')->table('wallet_history')
            ->select('utr_no')
            ->where(function($query) use ($request) {
                $query->where('transaction_id', $request->trnId)
                ->orWhere('utr_no', $request->trnId);
            })
            ->where('id','!=', $request->history_id)
            ->first();
            
        return response()->json(['isDuplicate' => $trnInfo ? true : false]);
    }

    public function check_duplicate_utr_edit(Request $request)
    {
        $utrInfo = DB::connection('sales_db')->table('wallet_history')
            ->select('utr_no')
            ->where(function($query) use ($request) {
                $query->where('utr_no', $request->utrNo)
                    ->orWhere('transaction_id', $request->utrNo);
            })
            ->where('id', '!=', $request->history_id)
            ->first();
        return response()->json(['isDuplicate' => $utrInfo ? true : false]);
    ...
    public function edit_new_wallet_amount(Request $request)
    {
        $input = $request->all();
        $validator = Validator::make($input, [
            'wallet_id'=>'required',
        ...
        $history_id = DB::connection('sales_db')->table('wallet_history')
        ->where('id',$request->history_id)
        ->update($walletHistoryArray);
        if($history_id)
        {
            return response()->json(['status'=>200,'message'=>'Payment details updated successfully. Waiting for financial approval. After approval, the amount will be reflected in your balance','data'=>$history_id]);
        }
        else
        {
            return response()->json(['status'=>201,'message'=>'Something went wrong. Please try agian after some time.']);
    ...
    public function get_wallet_history_by_id(Request $request)
    {
        $walletHistory = DB::connection('sales_db')->table('wallet_history')
        ->select(
            'id',
    ...
    public function check_last_guest_followup_details(Request $request)
    {
        $followupInfo = DB::connection('sales_db')->table('clients_followup_log as cfl')
        ->select('cfl.next_followup_date', 'cfl.created_by', 'cfl.client_id', 'cfl.followup_id',
        'cfl.created_date')
    ...
    public function get_tempo_package_redius(Request $request)
    {
        $tempoPackageRedius = DB::connection('sales_db')
        ->table('tempo_package_redius')
        ->select('id','redius_km')
        ->where('status',1)
        ->get();
        ...
        return response()->json([
            'status' => 200,
            'message' => 'Tempo Redius Details',
            'data'=> $tempoPackageRedius
        ]);
    }
