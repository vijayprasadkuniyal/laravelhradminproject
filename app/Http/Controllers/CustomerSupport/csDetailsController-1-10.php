...
class csDetailsController extends Controller
{
    ...
    //     ->select('enquiry_info.id','enquiry_info.created_date as recived_date','enquiry_info.event_date','enquiry_info.city_from','enquiry_info.city_to','enquiry_info.distance','enquiry_info.customer_id',
    //     'enquiry_info.no_of_attempt','customer_info.name','customer_info.primary_no','customer_info.email','group_names.name as group_name','product_service.service_name','product_category.category_name')
    ...
    //     ->when($engId, fn ($query) => $query->where('enquiry_info.id',$engId))
    ...
    //     if($engId=='')
    //     {
    //         $enqDetails->where('enquiry_info.enq_status',3);
    //     }
    ...
    {
        $engId = $request->enqId;
        $query = DB::connection('sales_db')->table('enquiry_info')
            ->select(
                'enquiry_info.id',
                'enquiry_info.created_date as received_date', // fixed typo 'recived_date' to 'received_date'
            ...
            ->leftJoin('product_category', 'product_category.id', '=', 'enquiry_info.category_id')
            ->leftJoin('group_names', 'group_names.group_id', '=', 'enquiry_info.group_id')
            ->groupBy('customer_info.id');

        if ($engId) {
            $query->where('enquiry_info.id', $engId);
            //$query->orWhere('enquiry_info.primary_no', $engId);
        }
        ...
        $enqDetails = $query->paginate($request->input('per_page', 10)); 

        return response()->json([
            'status' => 200,
            'message' => 'Not Sent Enquiry Details',
            'data' => $enqDetails->items(),
    ...
    public function get_return_lead(Request $request)
    {
        ...
        ->when($engId, fn ($query) => $query->where('rli.enq_id',$engId))
        ->orderBy('rli.id','DESC')
        ->paginate($request->per_page);
        if($engId=='')
        {
            $retuenLeadDetails->where('rli.status',0);
        }

        
        return response()->json(['status'=>200,'message'=>'Not Sent Enquiry Details','data'=>$retuenLeadDetails,'last_page'=>$retuenLeadDetails->lastPage()]);
