...
class EmpBasicinfoController extends Controller
{
    ...
    while ($reportingManagerId) {
      ...
      $probation_leave = LeaveType::where('is_probation_leave',1)->where('assign_when_request',0)->where('is_assign_quartly',1)->where('status',1)->get();
      ...
      foreach($probation_leave as $row){
        ...
        elseif ($diffInMonths==2){
         ...
         $data_array = array('emp_id'=>$rims_id,'leave_id'=>$row->id,
            ...
            'year'=>Carbon::now()->year,'total_leave'=>$total,'leave_taken'=>0,'remaining'=>$total,'is_probation_leave'=>1,'is_request_leave'=>0,'is_assign_quartly'=>1);
...
public function send_attendance_request(Request $request){
   ...
   public function save_attendance_regularize(Request $request){
    ...
    where('attendance_date',$request->date)->update(['status'=>$request->status]);
