...
$package_activation = $package_pending->whereIn('created_by', $emp_managers)
       ...
       //return 'kkk';
        ...
        $total_due_amount = $due_amount_data->where('created_by', $request->emp_id)
         ...
         $check_today_missing_target = DB::connection('sales_db')
                                     ->table('kra_and_kpi_details')->where('created_by',$request->emp_id)->whereDate('created_date',Carbon::now()->format('Y-m-d'))->sum('remaining_followup');
        $check_extra_followup = DB::connection('sales_db')
                                     ->table('kra_and_kpi_details')->where('created_by',$request->emp_id)->whereDate('created_date',Carbon::now()->format('Y-m-d'))->sum('extra_followup');
...
public function missing_kra_kpi_history(Request $request) {
    ...
    // Fetch employee managers and their reporting employees
    $emp_managers = DB::table('employee_managers')
        ->where(function($query) use ($request) {
            $query->whereRaw('FIND_IN_SET(?, reporting_to)', [$request->emp_id])
                  ->orWhere('dept_manager', $request->emp_id);
...
public function assigned_business_lead_to_manager(Request $request){
    ...
    'status'=>1,'created_by'=>$request->manager_id);
