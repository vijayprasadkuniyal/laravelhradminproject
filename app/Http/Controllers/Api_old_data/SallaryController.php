      ...
      foreach($attributes as $row){
        if($row->value!='' && $row->value!='null'){
          ...
          $data[] = array('emp_id'=>$request->emp_id,
            'attribute_id'=>$row->id,'amount_per_anum'=>$row->value,'amount_per_month'=>round($row->value/12),'status'=>1,'emp_package'=>$request->package_detail,'is_metrocity_emp'=>$request->metrocity,'is_deduction'=>$salary_attribute->is_checked,
                'is_contribute'=>$salary_attribute->for_contributtion);
      ...
      if($request->tds){
        ...
        $sallary_calculate = Sallary::updateOrInsert(
            ...
            [
                'emp_id'=>$request->emp_id,
                'attribute_id'=>12,
                'amount_per_anum'=>$request->tds*12,
                'amount_per_month'=>$request->tds,
                'status'=>1,
                'emp_package'=>$request->package_detail,
                'is_metrocity_emp'=>$request->metrocity,
                'is_deduction'=>$tds_attribute->is_checked,
                'is_contribute'=>$tds_attribute->for_contributtion,
              ]
          );
          DB::table('emp_salary_history')->where('emp_id',$request->emp_id)->where('attribute_id',12)->update(['status'=>0]);

          $data[] = array('emp_id'=>$request->emp_id,
            'attribute_id'=>12,'amount_per_anum'=>$request->tds*12,'amount_per_month'=>round($request->tds/12),'status'=>1,'emp_package'=>$request->package_detail,'is_metrocity_emp'=>$request->metrocity,'is_deduction'=>$tds_attribute->is_checked,
                'is_contribute'=>$tds_attribute->for_contributtion);
...
public function salary_calc(Request $request)
{
    ...
    $salary_attributes = Salaryattribute::where('is_checked', 0)
        ->where('for_contributtion',0)
        ->where('status', 1)
        ->where('attribute','!=','HRA')
        ->get();
...
public function get_tds_data($emp_id){
  ...
  $is_metrocity_emp = Sallary::where('emp_id',$emp_id)->orderBy('id','DESC')->where('status',1)->first();
  return response()->json(['status'=>200,'data'=> $data->amount_per_anum,'is_metro_city_emp'=> $is_metrocity_emp->is_metrocity_emp]);
