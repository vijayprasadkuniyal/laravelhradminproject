...
public function salary_calc(Request $request)
{
   ...
   //return  $salary_attribute;
    ...
    foreach ($salary_attribute as $row) {
        ...
        if ($row->deduction_from == 'Package' && $row->is_checked == 0 && $row->attribute!== 'TDS') {
            if ($row->percentage !== null && $row->amount === null) {
...
public function deduction_attribute(){
  $deduction = Salaryattribute::where('is_checked',1)->get();
  $attribute = [];
  foreach($deduction as $row){
  if($row->percentage!=''){
     $name = $row->attribute.' '.$row->percentage.' %';

  }
  if($row->amount!=''){
    $name = $row->attribute.' '.$row->amount.' RS';
