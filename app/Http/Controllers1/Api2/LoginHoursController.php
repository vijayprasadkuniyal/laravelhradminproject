...
class LoginHoursController extends Controller
{
    public function calculate_login_hours($emp_id){
      // LoginHours::where('created_date', '2023-11-28 17:38:16')->update(['created_date' => '2023-11-29 17:38:16']);
        $login_hours = LoginHours::where('emp_id', $emp_id)
      ...
      if(!$login_hours){
       ...
       $check_in =  $login_hours->login_time;
       $check_out = $login_hours->logout_time;
...
{
    $daily_attendance = LoginHours::orderBy('id','DESC')->where('emp_id', $id)->get();
    $data_array = [];
    if($daily_attendance){
      foreach ($daily_attendance as $row) {
        $date = Carbon::parse($row->created_date)->format('Y-m-d');
        $login_time = Carbon::parse($row->login_time)->format('H:i:s');
        $working_hours = '';
        $status = '';
        $sign_out = '';
        ...
        if ($row->logout_time == '') {
            ...
            $sign_out = Carbon::parse($row->logout_time)->format('H:i:s');
            $start = Carbon::parse($row->login_time)->toDateTimeString();
            $start = new Carbon($start);
            $end = Carbon::parse($row->logout_time)->toDateTimeString();
            $end = new Carbon($end);
            ...
            else if($diff_in_hour>=6 && $diff_in_hour<9){
              $status = "Half Day";
            }
            else if($diff_in_hour>=2 && $diff_in_hour<6){
              $status = "Short Leave";
            }
          ...
          $data_array[] = array('date'=>$date,'login_time'=>$login_time,'logout_time'=>$sign_out,'working_hours'=> $working_hours,'status'=>$status);
