...
class EmpBasicinfoController extends Controller
{
    public function save_employee(Request $request){
     ...
     if($employee_id){
      ...
      if($image = $request->file('pic')) {
         ...
         $basic_info->save();
         // $emp_details = array('employee_comp_id'=>$request->company,
         // 'branch_id'=>$request->branch,'employee_id'=>$rims_id,
         // 'employee_name'=>$request->first_name.$request->middle_name.$request->last_name,
         // 'employee_dept_id'=>$request->department,'employee_dept_name'=>$department_name[0],
         // 'employee_desg_id'=>$request->designation,'employee_desg_name'=>$designation_name[0],'created_by'=>1);
         // DB::connection('sales')->table('employee_details')->insert($emp_details);
         $basic_info->reporting_manager = $request->manager;
    ...
    while ($reportingManagerId) {
     ...
     $assign_leave = EmployeeLeaveBlance::where('quarter_no',Carbon::now()->quarter)->where('is_probation_leave',1)->where('emp_id',$rims_id)->where('year',Carbon::now()->year)->count('year');
     if($assign_leave==0){
      foreach($probation_leave as $row){
        ...
        elseif ($diffInMonths==2){
         ...
         $data_array = array('emp_id'=>$rims_id,'leave_id'=>$row->id,
            'quarter_no'=>Carbon::now()->quarter,'quarter_start_from'=>$current_date,'quarter_end'=>$endOfQuarter,
            'year'=>Carbon::now()->year,'total_leave'=>$total,'leave_taken'=>0,'remaining'=>$total,'is_probation_leave'=>1,'is_request_leave'=>0);
...
foreach ($emp_list as $basic_info) {
    $reportingManagerId = $basic_info->reporting_manager;
    $managerIds = [];
    ...
    DB::table('employee_managers')->insert([
        'emp_id' => $employeeId,
        'dept_manager' => $basic_info->reporting_manager,
        'reporting_to' => $reporting_to,
        'status' => 1,
        'from_date' => Carbon::now()->format('Y-m-d'),
        'dept_id' => $basic_info->dept_id,
        'designation_id' => $basic_info->desi_id,
    ]);
}
   ...
   public function department_based_employee(Request $request){
        ...
        $data = BasicInfo::where('emp_status',1)->whereIn('dept_id',$request->id)->get(['emp_id','emp_fname']);
   ...
   public function save_enquiry(Request $request){
    ...
    if($request->department && $request->employee){
      ...
      foreach($emp_id as $row){
        ...
        $data[] = array('selected_department'=>$emp_dept->dept_id,'selected_employee'=>$row,'message'=>$request->enquiry);
      ...
      foreach($dept_id as $dept){
        ...
        foreach($emp_id as $ids){
          $data[] = array('selected_department'=>$dept,'selected_employee'=>$ids->emp_id,'message'=>$request->enquiry);
